...
function positionSlotMultiplierMeter(meter = $("#slot-multiplier-meter")) {
  const stage = $("#screen-pvp .pharaoh-stage");
  const grid = $("#slot-grid");
  if (!meter || !stage || !grid) return;
  const stageRect = stage.getBoundingClientRect();
  const gridRect = grid.getBoundingClientRect();
  const meterWidth = meter.getBoundingClientRect().width || 158;
  const centeredRight = stageRect.right - gridRect.right + (gridRect.width - meterWidth) / 2;
  meter.style.setProperty("--slot-meter-top", `${Math.round(gridRect.top - stageRect.top - 35)}px`);
  meter.style.setProperty("--slot-meter-right", `${Math.round(centeredRight)}px`);
}
...
function renderSlotMultiplierMeter(slot = state.slot) {
  ...
  if (labelNode) labelNode.textContent = cascadeMode ? "МНОЖИТЕЛЬ КАСКАДА" : "ОБЩИЙ МНОЖИТЕЛЬ";
