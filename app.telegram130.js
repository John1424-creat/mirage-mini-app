...
class PharaohCanvasRenderer {
  ...
  drawCellFrame(x, y, w, h, symbol, options = {}) {
    ...
    outer.addColorStop(0, "rgba(255, 219, 135, 0.82)");
    outer.addColorStop(0.48, "rgba(255, 82, 190, 0.66)");
    outer.addColorStop(1, "rgba(110, 63, 159, 0.58)");
    const fill = ctx.createLinearGradient(sx, sy, sx + sw, sy + sh);
    fill.addColorStop(0, "rgba(255, 210, 126, 0.32)");
    fill.addColorStop(0.44, symbol.tier === "high" ? "rgba(220, 59, 151, 0.82)" : "rgba(172, 62, 151, 0.78)");
    fill.addColorStop(1, symbol.tier === "scatter" || symbol.tier === "multi" ? "rgba(255, 190, 99, 0.62)" : "rgba(39, 25, 92, 0.82)");
  ...
  drawSymbol(symbolId, x, y, w, h, options = {}) {
    ...
    const iconSize = Math.min(w, h) * (symbol.tier === "scatter" || symbol.tier === "multi" ? 0.82 : 0.78) * scale;
    ctx.save();
    ctx.globalAlpha = alpha;
    ctx.fillStyle = "rgba(0, 0, 0, 0.2)";
    ctx.beginPath();
    ctx.ellipse(cx, cy + iconSize * 0.35, iconSize * 0.36, iconSize * 0.1, 0, 0, Math.PI * 2);
    ctx.fill();
