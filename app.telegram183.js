...
function ensureSlotWinCeremonyOverlay() {
  ...
  const screen = $("#screen-pvp");
  if (!screen) return null;
  overlay = document.createElement("div");
  overlay.id = "slot-win-ceremony";
  overlay.className = "slot-win-ceremony";
...
function startSlotWinCeremony(type, amount, preferredDuration = 3000) {
  ...
  const draw = (now) => {
    ...
    if (alpha > 0) {
      const wash = ctx.createRadialGradient(width * 0.5, height * 0.36, 0, width * 0.5, height * 0.36, width * 0.72);
      wash.addColorStop(0, `rgba(255, 196, 91, ${0.08 * alpha})`);
      wash.addColorStop(0.45, `rgba(184, 38, 180, ${0.08 * alpha})`);
    ...
    items.forEach((item) => {
      ...
      if (y > safeFallBottom + item.size + 20) {
        if (progress < 0.76) {
          ...
          item.x = width * (0.04 + Math.random() * 0.92);
