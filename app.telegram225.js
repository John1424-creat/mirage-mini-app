...
const SLOT_CONFIG = {
  ...
  symbols: [
    ...
    { id: "scatter", label: "☀", name: "Дар джина", tier: "scatter", weight: 0.85, color: "#f4ccff", icon: "./assets/pharaoh-icons-premium/genie-scatter-v1.webp" },
...
function continueSlotFlowsAfterRound({ bonusBuy = false, isFreeSpin = false, presentation = null, duration = 0 } = {}) {
  const slot = state.slot;
  if (slot.bonusSessionActive && (bonusBuy || isFreeSpin)) {
...
function settlePharaohRoundResult(result) {
  ...
  if (result.freeSpinsAwarded > 0) {
    ...
    if (result.bonusGranted) beginSlotBonusSession(result.freeSpinsAwarded);
    else if (slot.bonusSessionActive) slot.bonusSessionInitialSpins += result.freeSpinsAwarded;
    else if (!result.freeSpinsPlayed) slot.freeSpinMultiplierBank = 0;
...
function initDebugSlotWinCeremony() {
  ...
  if (params.get("debugFreeSpins") === "1") {
    window.setTimeout(() => {
      switchTab("pvp");
      const slot = state.slot;
      slot.grid = [
