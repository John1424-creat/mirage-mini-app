...
class PharaohCanvasRenderer {
  ...
  drawSymbol(symbolId, x, y, w, h, options = {}) {
    ...
    const isScatter = symbol.id === "scatter";
    const isFeature = symbol.tier === "scatter" || symbol.tier === "multi";
    const iconRatio = symbol.tier === "scatter" ? 0.88 : symbol.tier === "multi" ? 0.74 : symbol.tier === "high" ? 0.84 : 0.82;
    const iconSize = Math.min(w, h) * iconRatio * scale;
    const iconCx = symbol.id === "multi" ? cx - iconSize * 0.03 : cx;
    const iconCy = symbol.id === "multi" ? cy - iconSize * 0.055 : cy;
    ctx.save();
    ctx.globalAlpha = alpha;
