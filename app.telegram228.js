...
const CARPET_LAUNCH_DURATION_MS = 720;
const CARPET_LAUNCH_X = 34;
const CARPET_LAUNCH_Y = -68;
...
function renderCarpet() {
  ...
  const hasLaunched = ["FLYING", "CASHED_OUT", "CRASHED"].includes(carpet.status);
  const x = hasLaunched ? CARPET_LAUNCH_X + Math.pow(progress, 0.72) * (184 - CARPET_LAUNCH_X) : 0;
  const y = hasLaunched ? CARPET_LAUNCH_Y + Math.pow(progress, 1.28) * (-226 - CARPET_LAUNCH_Y) : 0;
  const scale = hasLaunched ? 1.02 - progress * 0.02 : 1;
  const rotate = hasLaunched ? -10 + progress * 14 : -8;

  stage.style.setProperty("--carpet-progress", progress.toFixed(4));
  stage.classList.toggle("ready", carpet.status === "READY");
  stage.classList.toggle("launching", carpet.status === "STARTING");
  ...
  if (screen) {
    ...
    screen.classList.toggle("carpet-launching", carpet.status === "STARTING");
  ...
  if (status) {
    if (carpet.status === "STARTING") status.textContent = "ВЗЛЕТ";
  ...
  if (label) {
    ...
    else if (carpet.status === "STARTING") label.textContent = "ВЗЛЕТ";
  ...
  if (button) {
    button.classList.toggle("launching", carpet.status === "STARTING");
    button.classList.toggle("flying", carpet.status === "FLYING");
    button.classList.toggle("lost", carpet.status === "CRASHED");
    button.disabled = carpet.status === "STARTING" && !carpet.autoRunning;
  }
...
function handleCarpetMainButton() {
  ...
  if (state.carpet.status === "FLYING") cashoutCarpetRound();
  else if (state.carpet.status === "STARTING") return;
