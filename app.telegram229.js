...
const CARPET_ACCELERATION_MS = 720;
const CARPET_ACCELERATION_TIME_CONSTANT_MS = 220;
const CARPET_VISUAL_X_SPEED = 0.018;
const CARPET_VISUAL_Y_SPEED = 0.022;
const CARPET_VISUAL_MAX_X = 184;
const CARPET_VISUAL_MAX_Y = -226;
...
function getCarpetVisualPose(carpet, now = performance.now()) {
  if (carpet.status === "READY" || !carpet.roundStartedAt) {
    return { x: 0, y: 0, scale: 1, rotate: -8, speed: 0, accelerating: false };
  }
  ...
  const speed = 1 - Math.exp(-elapsed / CARPET_ACCELERATION_TIME_CONSTANT_MS);
  const travel = elapsed - CARPET_ACCELERATION_TIME_CONSTANT_MS * speed;
  const x = Math.min(CARPET_VISUAL_MAX_X, travel * CARPET_VISUAL_X_SPEED);
  const y = Math.max(CARPET_VISUAL_MAX_Y, -travel * CARPET_VISUAL_Y_SPEED);
  const distanceProgress = Math.min(1, Math.hypot(x, y) / Math.hypot(CARPET_VISUAL_MAX_X, CARPET_VISUAL_MAX_Y));

  return {
    x,
    y,
    scale: 1 - distanceProgress * 0.02,
    rotate: -8 + distanceProgress * 12,
    speed,
...
function renderCarpet() {
  ...
  const { x, y, scale, rotate, speed, accelerating } = pose;
  carpet.visualX = x;
  carpet.visualY = y;
