...
function renderSlotPaytable() {
  ...
  [
    ...
    },
    {
      id: "multi",
      title: "Множитель",
      description: "Показывает x2 / x3 / x5 / x10 и активируется на выигрышном каскаде; во free spins пополняет общий множитель до x10.",
    },
  ].forEach((feature) => {
...
function updateSlotStatus(slot = state.slot, message = slot.message) {
  ...
  const bank = Math.max(1, slot.freeSpinMultiplierBank || 0);
  const meter = bonusActive ? `МНОЖИТЕЛЬ x${bank}` : "";
  status.textContent = visibleMessage && meter ? `${visibleMessage} · ${meter}` : visibleMessage || meter;
...
function buildSlotStepExplanation(step) {
  ...
  return {
    mode: "explain",
    summary: `${primary.count} ${symbolName} • ${baseMultiplier} • ${appliedMultiplier}`,
    items: [
      ...
      { text: appliedMultiplier, title: "Множитель каскада" },
      { text: `+${format(step.win)}`, title: "Выигрыш каскада" },
    ],
    cascadeCount,
  };
}
