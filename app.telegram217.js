...
function renderPharaoh() {
  ...
  if (bonus) {
    ...
    bonus.title = !SLOT_CONFIG.bonusBuy.enabled ? "Бонус будет включен позже"
      : !bonusWithinPool ? `Лимит покупки: до ${format(bonusBuyLimit)} 💎 при пуле ${format(SLOT_CONFIG.roomPoolUsd)}`
      : `Купить 15 free spins за ${format(bonusCost)} 💎`;
  }
  ...
  if (bonusNote) {
    bonusNote.textContent = bonusWithinPool
      ? `Ты покупаешь вход в бонус: ${SLOT_CONFIG.bonusBuy.freeSpins} free spins за ${SLOT_CONFIG.bonusBuy.costMultiplier} ставок. Ставка ${format(slot.stake)} 💎, цена серии ${format(bonusCost)} 💎.`
      : `Покупка недоступна: цена серии ${format(bonusCost)} 💎 выше лимита пула ${format(bonusBuyLimit)} 💎. Уменьши ставку.`;
  }
