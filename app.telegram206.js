...
function getSlotWinCeremonyConfig(type) {
  if (type === "legend") {
    return {
      ...
      storm: 0.58,
      scale: [0.62, 1.04],
      mix: ["coin", "ruby", "blueGem", "greenGem", "scarab", "sun", "scroll", "ankh", "orb"],
    };
  }
  if (type === "mega" || type === "free-win") {
    return {
      ...
      storm: 0.46,
      scale: [0.56, 0.94],
      mix: ["coin", "ruby", "blueGem", "greenGem", "scarab", "sun"],
    };
  }
  if (type === "big") {
    return {
      ...
      storm: 0.28,
      scale: [0.48, 0.82],
      mix: ["coin", "ruby", "blueGem", "greenGem"],
    };
  }
  return null;
}
...
function drawSlotWinTreasureParticle(ctx, x, y, size, rotation, alpha, kind = "coin", depth = 1) {
  ...
  ctx.globalAlpha = alpha;
  ctx.globalCompositeOperation = "source-over";
  ctx.shadowColor = kind === "ruby" ? "rgba(255, 73, 205, 0.62)" : "rgba(255, 215, 105, 0.56)";
  ctx.shadowBlur = Math.max(5, size * 0.32 * depth);
...
function startSlotWinCeremony(type, amount, preferredDuration = 3000) {
  ...
  for (let index = 0; index < total; index += 1) {
    ...
    const depth = 0.72 + Math.random() * 0.72;
    const kind = config.mix[Math.floor(Math.random() * config.mix.length)] || "coin";
    const sizeBase = 16 + Math.random() * 18;
    const lane = index % 2 === 0 ? -1 : 1;
    items.push({
      kind,
      depth,
      x: width * (0.5 + lane * (0.18 + Math.random() * 0.32)),
      y: protectedTop - height * (0.06 + Math.random() * 0.46),
      vx: lane * (4 + Math.random() * 14) + (Math.random() - 0.5) * 12,
      vy: height * (0.13 + Math.random() * 0.16 + config.storm * 0.04) * depth,
      size: sizeBase * (config.scale[0] + Math.random() * (config.scale[1] - config.scale[0])),
      spin: (Math.random() - 0.5) * 6,
      rotation: Math.random() * Math.PI * 2,
      delay: spread * duration * 0.58 + Math.random() * 220,
    ...
    items.forEach((item) => {
      ...
      if (y > safeFallBottom + item.size + 20) {
        if (progress < 0.76) {
          ...
          item.depth = 0.72 + Math.random() * 0.72;
          item.y = protectedTop - item.size - Math.random() * height * 0.24;
          item.x = width * (0.5 + (Math.random() < 0.5 ? -1 : 1) * (0.18 + Math.random() * 0.32));
          item.delay = elapsed + Math.random() * 120;
          item.vx = (Math.random() - 0.5) * (12 + item.depth * 16);
          item.vy = height * (0.13 + Math.random() * 0.16 + config.storm * 0.04) * item.depth;
        }
        return;
      }
      if (y < protectedTop) return;
      const itemAlpha = Math.min(1, t / 0.08) * alpha;
      const presentationFade = Math.abs(y - focusY) < 62 && Math.abs(x - width * 0.5) < Math.min(width * 0.34, 170) ? 0.2 : 1;
      ...
      const frontAlpha = itemAlpha * bottomFade * topFade * presentationFade * Math.min(1, 0.66 + item.depth * 0.2);
