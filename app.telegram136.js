...
class PharaohCanvasRenderer {
  ...
  drawCellFrame(x, y, w, h, symbol, options = {}) {
    ...
    outer.addColorStop(0, isPremium ? "rgba(255, 228, 150, 0.9)" : "rgba(255, 210, 128, 0.62)");
    outer.addColorStop(0.45, isFeature ? "rgba(255, 91, 213, 0.78)" : "rgba(212, 75, 169, 0.54)");
    outer.addColorStop(1, "rgba(85, 60, 132, 0.42)");
    const fill = ctx.createRadialGradient(cx, cy - sh * 0.16, sw * 0.08, cx, cy + sh * 0.2, sw * 0.72);
    fill.addColorStop(0, isFeature ? "rgba(255, 196, 102, 0.34)" : "rgba(255, 203, 137, 0.2)");
    fill.addColorStop(0.46, isPremium ? "rgba(82, 36, 103, 0.78)" : "rgba(55, 32, 93, 0.76)");
    fill.addColorStop(1, "rgba(20, 14, 46, 0.88)");

    ctx.save();
    ctx.globalAlpha = alpha;
    if (winning) {
      ctx.globalCompositeOperation = "lighter";
      ctx.shadowColor = "rgba(255, 224, 142, 0.9)";
      ctx.shadowBlur = 18 + shimmer * 10;
      ctx.fillStyle = "rgba(255, 215, 126, 0.24)";
    ...
    ctx.shadowBlur = 7;
    ctx.fillStyle = "rgba(14, 9, 34, 0.68)";
    this.roundRect(sx - 1, sy + 1, sw + 2, sh + 1, radius + 1);
    ctx.fill();
    ...
    gloss.addColorStop(0.22, "rgba(255, 255, 255, 0.045)");
    gloss.addColorStop(1, "rgba(0, 0, 0, 0.22)");
    ctx.fillStyle = gloss;
    this.roundRect(sx + 2, sy + 2, sw - 4, sh - 4, radius - 2);
    ctx.fill();

    ctx.strokeStyle = winning ? "rgba(255, 235, 163, 0.98)" : isPremium ? "rgba(255, 214, 125, 0.68)" : "rgba(255, 199, 117, 0.42)";
    ctx.lineWidth = winning ? 2.2 : 1;
  ...
  drawSymbol(symbolId, x, y, w, h, options = {}) {
    ...
    const iconSize = Math.min(w, h) * (symbol.tier === "scatter" || symbol.tier === "multi" ? 0.98 : 0.92) * scale;
    ctx.save();
    ctx.globalAlpha = alpha;
    ctx.fillStyle = "rgba(0, 0, 0, 0.22)";
    ctx.beginPath();
    ctx.ellipse(cx, cy + iconSize * 0.34, iconSize * 0.34, iconSize * 0.09, 0, 0, Math.PI * 2);
    ctx.fill();

    ctx.shadowColor = winning ? "rgba(255, 239, 169, 0.98)" : symbol.color;
    ctx.shadowBlur = winning ? 18 : 5;
    if (img) {
...
function getSlotWinPresentation(result, { bonusBuy = false, isFreeSpin = false } = {}) {
  if (bonusBuy) {
    ...
    return {
      ...
      note: result.totalWin > 0 ? `Итог +${format(result.totalWin)} ✦` : "Джин запускает серию",
    };
  }
  ...
  if (multiplier >= 100) {
    return { type: "legend", title: "LEGEND WIN", value: `+${format(result.totalWin)} ✦`, note: `${floorMultiplier(multiplier).toFixed(2)}x` };
  }
  if (multiplier >= 30) {
    return { type: "mega", title: "MEGA WIN", value: `+${format(result.totalWin)} ✦`, note: `${floorMultiplier(multiplier).toFixed(2)}x` };
  }
  if (multiplier >= 10) {
    return { type: "big", title: "BIG WIN", value: `+${format(result.totalWin)} ✦`, note: `${floorMultiplier(multiplier).toFixed(2)}x` };
  }
