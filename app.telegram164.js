...
class PharaohCanvasRenderer {
  ...
  async animateReel(targetGrid, context = "base", duration = 760) {
      ...
      for (let col = 0; col < cols; col += 1) {
        for (let row = rows - 1; row >= 0; row -= 1) {
          ...
          outgoing.push({
            ...
            row,
            col,
            delay: (rows - 1 - row) * 0.026 + col * 0.012,
    ...
    await this.animate(duration, (progress) => {
      this.drawBackground();
      const ctx = this.ctx;
      if (outgoing.length && progress < 0.78) {
        outgoing.forEach((sprite) => {
          const local = Math.max(0, Math.min(1, (progress - sprite.delay) / 0.62));
          if (local <= 0) {
      ...
      incoming.forEach((sprite) => {
        const entryStart = outgoing.length ? 0.54 : 0;
        const delay = entryStart + sprite.delay * (outgoing.length ? 0.58 : 1);
        const local = Math.max(0, Math.min(1, (progress - delay) / Math.max(0.28, 1 - delay)));
        if (local <= 0) return;
  ...
  try {
    ...
    if (canvasRenderer) {
      ...
      await pharaohCanvasRenderer.playResult(
        ...
        {
          ...(ultraLiteRenderer
            ? { reel: 820, drop: 220, win: 560, cascade: 980, empty: 70 }
            : liteRenderer ? { reel: 920, drop: 280, win: 700, cascade: 1040, empty: 90 } : { reel: 980, win: 780, cascade: 1100 }),
          onStep: (step) => {
            if (slot.spinRunId !== runId) return;
