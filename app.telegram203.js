...
const SLOT_GENIE_REACTION_ASSETS = {
  idle: "./assets/pharaoh-genie-premium-idle-smoke-fit-v1.png",
  casting: "./assets/pharaoh-genie-premium-spin-safe-v1.png",
  spin: "./assets/pharaoh-genie-premium-spin-safe-v1.png",
  cascade: "./assets/pharaoh-genie-premium-win-safe-v1.png",
  win: "./assets/pharaoh-genie-premium-win-safe-v1.png",
  big: "./assets/pharaoh-genie-premium-legend-safe-v1.png",
  mega: "./assets/pharaoh-genie-premium-legend-safe-v1.png",
  legend: "./assets/pharaoh-genie-premium-legend-safe-v1.png",
  free: "./assets/pharaoh-genie-premium-win-safe-v1.png",
};
...
function positionSlotPresentationPanel(panel) {
  ...
  const width = Math.min(gridRect.width - 66, viewportWidth - 92, 286);
...
function getSlotWinCeremonyConfig(type) {
  if (type === "legend") {
    return {
      ...
      storm: 0.84,
      scale: [0.72, 1.18],
      mix: ["coin", "coin", "ruby", "blueGem", "greenGem", "scarab", "sun", "scroll", "ankh", "orb"],
    };
  }
  if (type === "mega" || type === "free-win") {
    return {
      ...
      storm: 0.62,
      scale: [0.62, 1.04],
      mix: ["coin", "coin", "coin", "ruby", "blueGem", "greenGem", "scarab", "sun"],
    };
  }
  if (type === "big") {
    return {
      ...
      storm: 0.32,
      scale: [0.54, 0.92],
      mix: ["coin", "coin", "coin", "ruby", "blueGem", "greenGem"],
    };
  }
  return null;
}
...
function startSlotWinCeremony(type, amount, preferredDuration = 3000) {
  ...
  const safeFallBottom = controlsRect
    ? Math.max(height * 0.52, Math.min(height * 0.88, controlsRect.top - rect.top - 14))
    : height * 0.86;
  const items = [];
  ...
  for (let index = 0; index < total; index += 1) {
    ...
    items.push({
      kind,
      depth,
      x: width * (-0.08 + Math.random() * 1.16),
      y: -height * (0.04 + Math.random() * 0.86),
      vx: (Math.random() - 0.5) * (18 + depth * 22),
      vy: height * (0.16 + Math.random() * 0.19 + config.storm * 0.06) * depth,
      size: sizeBase * (config.scale[0] + Math.random() * (config.scale[1] - config.scale[0])),
      spin: (Math.random() - 0.5) * 6,
      rotation: Math.random() * Math.PI * 2,
      delay: spread * duration * 0.48 + Math.random() * 240,
  ...
  const draw = (now) => {
    ...
    items.forEach((item) => {
      ...
      const y = item.y + item.vy * (local / 1000) * 3.05;
      ...
      if (y > safeFallBottom + item.size + 20) {
        if (progress < 0.76) {
          ...
          item.y = -item.size - Math.random() * height * 0.28;
          item.x = width * (-0.08 + Math.random() * 1.16);
          item.delay = elapsed + Math.random() * 120;
          item.vx = (Math.random() - 0.5) * (18 + item.depth * 22);
          item.vy = height * (0.16 + Math.random() * 0.19 + config.storm * 0.06) * item.depth;
        }
        return;
      }
      const itemAlpha = Math.min(1, t / 0.08) * alpha;
      const bottomFade = y > safeFallBottom - 64 ? Math.max(0, 1 - (y - (safeFallBottom - 64)) / 64) : 1;
      const frontAlpha = itemAlpha * bottomFade * Math.min(1, 0.74 + item.depth * 0.22);
