...
function createSlotSpinResult(stake, context = "base", { freeMultiplierBank = 0 } = {}) {
  ...
  while (cascadeIndex < 10) {
    ...
    if (context === "free" && evaluation.multiplierCount > 0) {
      for (let i = 0; i < evaluation.multiplierCount; i += 1) {
        multiplierAdded += getSlotFreeSpinMultiplierValue();
      }
    ...
    const randomMultiplier = evaluation.wins.length > 0 && evaluation.multiplierCount > 0 && context !== "free"
      ? evaluation.multiplierCount * (2 + Math.floor(Math.random() * 4))
      : 0;
...
function renderSlotPaytable() {
  ...
  [
    ...
    },
    {
      id: "multi",
      title: "Множитель",
      description: "Усиливает выигрышный каскад; во free spins пополняет накопительный множитель.",
    },
  ].forEach((feature) => {
...
function renderPharaoh() {
  ...
  if (status) {
    const statusMessage = getSlotVisibleStatusMessage(slot.message);
    status.textContent = statusMessage;
    status.classList.toggle("slot-status-empty", !statusMessage);
  }
  renderSlotPresentation(slot);
      ...
      await pharaohCanvasRenderer.playResult(
        ...
        bonusBuy || isFreeSpin ? "free" : "base",
        {
          ...
          onStep: (step) => {
            ...
            if (step.phase === "free") slot.freeSpins = freeSpinBank;
...
function initDebugSlotWinCeremony() {
  ...
  if (params.get("debugMultiplier") === "1") {
    window.setTimeout(() => {
      ...
      const slot = state.slot;
      slot.grid = [
        ["ankh", "scarab", "blue", "multi", "coin", "ruby"],
