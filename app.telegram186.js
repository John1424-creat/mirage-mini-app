...
function ensureSlotPresentationPanel() {
  ...
  const host = $(".app-shell") || $(".pharaoh-stage");
  if (!host) return null;
...
function positionSlotPresentationPanel(panel) {
  ...
  if (!panel || !shell || !gridRoot) return;
  const shellRect = shell.getBoundingClientRect();
  const gridRect = gridRoot.getBoundingClientRect();
  if (!shellRect.width || !gridRect.width) return;
  const top = gridRect.top - shellRect.top + gridRect.height * 0.24;
  const width = Math.min(gridRect.width + 2, shellRect.width - 56);
...
function ensureSlotWinCeremonyOverlay() {
  ...
  const host = $(".app-shell") || document.body;
  if (!host) return null;
