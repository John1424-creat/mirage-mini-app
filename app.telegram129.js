...
class PharaohCanvasRenderer {
  ...
  resize() {
    ...
    if (this.lastGrid) this.drawGrid(this.lastGrid, { winningKeys: this.lastWinningKeys });
  }
  ...
  drawBackground() {
    ...
    const ctx = this.ctx;
    this.clear();
    const bg = ctx.createRadialGradient(this.width * 0.5, this.height * 0.34, 5, this.width * 0.5, this.height * 0.42, this.width * 0.62);
    bg.addColorStop(0, "rgba(255, 202, 102, 0.2)");
    bg.addColorStop(0.48, "rgba(76, 30, 88, 0.44)");
    bg.addColorStop(1, "rgba(12, 7, 29, 0.78)");
    ctx.fillStyle = bg;
  ...
  drawCellFrame(x, y, w, h, symbol, options = {}) {
    const ctx = this.ctx;
    const { winning = false, alpha = 1, scale = 1 } = options;
    const cx = x + w / 2;
    const cy = y + h / 2;
    const sw = w * scale;
    const sh = h * scale;
    const sx = cx - sw / 2;
    const sy = cy - sh / 2;
    const radius = Math.max(6, Math.min(10, sw * 0.14));
    const gradient = ctx.createLinearGradient(sx, sy, sx + sw, sy + sh);

    if (symbol.tier === "high") {
      gradient.addColorStop(0, "rgba(255, 99, 190, 0.92)");
      gradient.addColorStop(1, "rgba(98, 36, 126, 0.9)");
    } else if (symbol.tier === "mid") {
      gradient.addColorStop(0, "rgba(255, 210, 111, 0.86)");
      gradient.addColorStop(1, "rgba(97, 45, 124, 0.88)");
    } else if (symbol.tier === "scatter" || symbol.tier === "multi") {
      gradient.addColorStop(0, "rgba(255, 226, 142, 0.94)");
      gradient.addColorStop(1, "rgba(230, 71, 184, 0.86)");
    } else {
      gradient.addColorStop(0, "rgba(226, 88, 180, 0.86)");
      gradient.addColorStop(1, "rgba(59, 42, 118, 0.88)");
    }

    ctx.save();
    ctx.globalAlpha = alpha;
    ctx.shadowColor = winning ? "rgba(255, 216, 120, 0.74)" : "rgba(0, 0, 0, 0.28)";
    ctx.shadowBlur = winning ? 18 : 6;
    ctx.fillStyle = "#251334";
    this.roundRect(sx, sy, sw, sh, radius);
    ctx.fill();
    ctx.fillStyle = gradient;
    this.roundRect(sx + 1, sy + 1, sw - 2, sh - 2, radius - 1);
    ctx.fill();
    ctx.strokeStyle = winning ? "rgba(255, 229, 149, 0.95)" : "rgba(255, 211, 126, 0.5)";
  ...
  drawSymbol(symbolId, x, y, w, h, options = {}) {
    ...
    const ctx = this.ctx;
    const { winning = false, alpha = 1, scale = 1, offsetY = 0 } = options;
    this.drawCellFrame(x, y + offsetY, w, h, symbol, { winning, alpha, scale });

    const cx = x + w / 2;
    const cy = y + offsetY + h / 2;
    const img = this.images.get(symbol.id);
    const iconSize = Math.min(w, h) * (symbol.tier === "scatter" || symbol.tier === "multi" ? 0.76 : 0.72) * scale;
    ctx.save();
    ctx.globalAlpha = alpha;
    ctx.shadowColor = winning ? "rgba(255, 239, 169, 0.95)" : symbol.color;
    ctx.shadowBlur = winning ? 16 : 7;
    if (img) {
  ...
  drawWinLinks(winningKeys) {
    ...
    for (let pass = 0; pass < 2; pass += 1) {
      ...
      ctx.strokeStyle = pass === 0 ? "rgba(255, 219, 121, 0.42)" : "rgba(255, 91, 207, 0.68)";
  ...
  drawGrid(grid, options = {}) {
    ...
    if (this.lastWinningKeys.size > 1) this.drawWinLinks(this.lastWinningKeys);
  }
  ...
  async animateReel(targetGrid, context = "base", duration = 760) {
    ...
    const columnRandoms = Array.from({ length: cols }, () =>
      Array.from({ length: rows + 3 }, () => getSlotWeightedSymbol(context).id)
    );
    await this.animate(duration, (progress) => {
      ...
      for (let col = 0; col < cols; col += 1) {
        ...
        const offset = (1 - Math.pow(1 - local, 2)) * (cellH + gap) * 2.2;
        for (let row = -2; row < rows; row += 1) {
          const symbolId = local > 0.86 && row >= 0 ? targetGrid[row][col] : columnRandoms[col][row + 2];
          const x = pad + col * (cellW + gap);
          const y = pad + row * (cellH + gap) + offset % (cellH + gap);
          this.drawSymbol(symbolId, x, y, cellW, cellH, { alpha: row < 0 ? 0.5 : 1, scale: 0.96 + ease * 0.04 });
        }
      }
    });
    ...
    await this.animate(duration, (progress) => {
      ...
      grid.forEach((row, rowIndex) => {
        row.forEach((symbolId, colIndex) => {
          ...
          this.drawSymbol(symbolId, x, y, cellW, cellH, { offsetY, alpha: 0.45 + ease * 0.55 });
        });
      });
    });
    this.drawGrid(grid);
  ...
  async animateWins(grid, winningKeys, duration = 620, emptyDelay = 120) {
    await this.preload();
    ...
    await this.animate(duration, (progress) => {
      ...
      grid.forEach((row, rowIndex) => {
        row.forEach((symbolId, colIndex) => {
          ...
          const x = pad + colIndex * (cellW + gap);
          const y = pad + rowIndex * (cellH + gap);
          this.drawSymbol(symbolId, x, y, cellW, cellH, { winning: keys.has(key), scale: keys.has(key) ? pulse : 1 });
        });
      });
      this.drawWinLinks(keys);
    });
  }
  ...
  async playResult(displaySteps, context = "base", timings = {}) {
    ...
    for (const [index, step] of displaySteps.entries()) {
      ...
      await this.animateWins(step.grid, step.winningKeys, winDuration, emptyDelay);
    }
  }
}
...
function getSlotReactionContent(slot) {
  ...
  if (slot.status !== "READY") {
    ...
    return { state: "spin", text: "МАГИЯ СПИНА" };
  }
  if (slot.totalWin > 0) return { state: "win", text: "ДЖИН ВЫИГРЫШ" };
...
function initEvents() {
  ...
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && state.slot.rulesOpen) closeSlotRules();
  });
