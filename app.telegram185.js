...
function animateSlotPresentationValue(presentation, duration = 900) {
  ...
  if (!target) return;
  const startedAt = performance.now();
  const countDuration = Math.min(1150, Math.max(520, duration * 0.68));
  const tick = (now) => {
...
function showSlotPresentation(presentation, duration = 1550) {
  ...
  activateSlotPrizeFrame(slot.presentation.type, duration);
  if (SLOT_WIN_CEREMONY_TIERS.has(slot.presentation.type)) {
    startSlotWinCeremony(slot.presentation.type, slot.presentation.win, duration);
  }
  animateSlotPresentationValue(activePresentation, duration);
