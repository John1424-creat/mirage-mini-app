...
function getSlotWinCeremonyConfig(type) {
  if (type === "legend") {
    return {
      ...
      storm: 0.82,
      scale: [0.52, 1.12],
      mix: ["dust", "dust", "coin", "coin", "ruby", "blueGem", "greenGem", "scarab", "sun", "scroll", "ankh", "orb"],
    };
  }
  if (type === "mega" || type === "free-win") {
    return {
      ...
      storm: 0.66,
      scale: [0.48, 0.98],
      mix: ["dust", "coin", "coin", "ruby", "blueGem", "greenGem", "scarab", "sun"],
    };
  }
  if (type === "big") {
    return {
      ...
      storm: 0.5,
      scale: [0.44, 0.9],
      mix: ["dust", "coin", "coin", "ruby", "blueGem", "greenGem"],
    };
  }
  return null;
}
...
function startSlotWinCeremony(type, amount, preferredDuration = 3000) {
  ...
  for (let index = 0; index < total; index += 1) {
    ...
    const depth = 0.56 + Math.random() * 0.92;
    const kind = config.mix[Math.floor(Math.random() * config.mix.length)] || "coin";
    const isDust = kind === "dust";
    const sizeBase = isDust ? 5 + Math.random() * 7 : 15 + Math.random() * 20;
    const sideBias = index % 5 === 0 ? (Math.random() < 0.5 ? width * 0.08 : width * 0.92) : width * (0.05 + Math.random() * 0.9);
    items.push({
      kind,
      depth,
      x: sideBias,
      ...
      vy: height * (0.12 + Math.random() * 0.18 + config.storm * 0.05) * depth,
      size: sizeBase * (config.scale[0] + Math.random() * (config.scale[1] - config.scale[0])),
      spin: (Math.random() - 0.5) * 6,
      rotation: Math.random() * Math.PI * 2,
      delay: spread * duration * 0.38 + Math.random() * 160,
    ...
    items.forEach((item) => {
      ...
      if (y > safeFallBottom + item.size + 20) {
        if (progress < 0.76) {
          ...
          item.depth = 0.56 + Math.random() * 0.92;
          item.y = protectedTop - item.size - Math.random() * height * 0.18;
          item.x = width * (0.05 + Math.random() * 0.9);
          item.delay = elapsed + Math.random() * 90;
          item.vx = (Math.random() - 0.5) * (10 + config.storm * 22);
          item.vy = height * (0.12 + Math.random() * 0.18 + config.storm * 0.05) * item.depth;
        }
        return;
      }
...
function getSlotPresentationDuration(presentation = {}, requestedDuration = 1550) {
  ...
  if (type === "legend") return Math.max(requestedDuration, 7200);
  if (type === "mega" || type === "free-win") return Math.max(requestedDuration, 5400);
  if (type === "big") return Math.max(requestedDuration, 4200);
...
function getSlotPresentationCountDuration(presentation = {}, totalDuration = 1550) {
  const type = presentation.type || "idle";
  if (type === "legend") return Math.min(totalDuration - 1280, 5800);
  if (type === "mega" || type === "free-win") return Math.min(totalDuration - 980, 4400);
  if (type === "big") return Math.min(totalDuration - 760, 3300);
