...
function getSlotWinPresentation(result, { bonusBuy = false, isFreeSpin = false } = {}) {
  ...
  if (multiplier >= 100) {
    return { type: "legend", title: "ЛЕГЕНДАРНО", value: `+${format(result.totalWin)} 💎`, note };
  }
  if (multiplier >= 30) {
    return { type: "mega", title: "МЕГА ВЫИГРЫШ", value: `+${format(result.totalWin)} 💎`, note };
  }
  if (multiplier >= 10) {
    return { type: "big", title: "БОЛЬШОЙ ВЫИГРЫШ", value: `+${format(result.totalWin)} 💎`, note };
  }
  return { type: "win", title: "ВЫИГРЫШ", value: `+${format(result.totalWin)} 💎`, note };
}
