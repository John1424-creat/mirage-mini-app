...
function getSlotWinCeremonyConfig(type) {
  if (type === "legend") {
    return { tier: "legend", coins: 92, rubies: 18, lightning: 5, duration: 3800, storm: 1 };
  }
  if (type === "mega" || type === "free-win") {
    return { tier: type === "free-win" ? "mega" : "mega", coins: 72, rubies: 12, lightning: 3, duration: 3300, storm: 0.7 };
  }
  if (type === "big") {
    return { tier: "big", coins: 48, rubies: 7, lightning: 0, duration: 2800, storm: 0.35 };
  }
  return null;
}

function drawSlotWinCoin(ctx, x, y, size, rotation, alpha, kind = "coin") {
  ctx.save();
  ctx.translate(x, y);
  ctx.rotate(rotation);
  ctx.globalAlpha = alpha;
  if (kind === "ruby") {
    const ruby = ctx.createLinearGradient(-size, -size, size, size);
    ruby.addColorStop(0, "#fff1ff");
    ruby.addColorStop(0.24, "#ff7ddb");
    ruby.addColorStop(0.58, "#d8198e");
    ruby.addColorStop(1, "#690b66");
    ctx.fillStyle = ruby;
    ctx.shadowColor = "rgba(255, 69, 200, 0.72)";
    ...
    const coin = ctx.createRadialGradient(-size * 0.25, -size * 0.32, 0, 0, 0, size);
    coin.addColorStop(0, "#fff6bc");
    coin.addColorStop(0.34, "#ffd95a");
    coin.addColorStop(0.72, "#d88412");
    coin.addColorStop(1, "#704008");
    ctx.scale(1, 0.72 + Math.abs(Math.sin(rotation)) * 0.28);
    ctx.shadowColor = "rgba(255, 214, 91, 0.68)";
    ctx.shadowBlur = size * 0.6;
    ctx.beginPath();
    ctx.fillStyle = coin;
    ctx.arc(0, 0, size, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = "rgba(255, 246, 177, 0.9)";
    ctx.lineWidth = Math.max(0.9, size * 0.13);
    ctx.stroke();
    ctx.beginPath();
    ctx.strokeStyle = "rgba(111, 63, 5, 0.34)";
...
function startSlotWinCeremony(type, amount, preferredDuration = 3000) {
  ...
  const items = [];
  const total = config.coins + config.rubies;
  for (let index = 0; index < total; index += 1) {
    const ruby = index >= config.coins;
    const spread = index / Math.max(1, total - 1);
    items.push({
      kind: ruby ? "ruby" : "coin",
      x: width * (0.04 + Math.random() * 0.92),
      y: -height * (0.04 + Math.random() * 0.86),
      vx: (Math.random() - 0.5) * (ruby ? 18 : 26),
      vy: height * (0.22 + Math.random() * 0.28 + config.storm * 0.07),
      size: ruby ? 5.5 + Math.random() * 3.5 : 5.8 + Math.random() * 5.5,
      spin: (Math.random() - 0.5) * 6,
      rotation: Math.random() * Math.PI * 2,
      delay: spread * duration * 0.44 + Math.random() * 280,
  ...
  const draw = (now) => {
    ...
    items.forEach((item) => {
      ...
      const y = item.y + item.vy * (local / 1000) * 2.9;
      const x = item.x + Math.sin(item.drift + local / 360) * 18 + item.vx * (local / 1000);
      if (y > height + 40) {
        if (progress < 0.76) {
          item.y = -30 - Math.random() * height * 0.36;
      ...
      drawSlotWinCoin(ctx, x, y, item.size, item.rotation + local * item.spin * 0.0012, itemAlpha, item.kind);
    });
