...
function renderPharaoh() {
  ...
  if (freeSpins) {
    const freeText = slot.bonusSessionActive ? `FS ${slot.freeSpins}` : slot.freeSpins > 0 ? `FREE SPINS ${slot.freeSpins}` : "FREE SPINS 0";
    const autoText = slot.bonusSessionActive ? `${slot.bonusSessionPlayed}/${slot.bonusSessionInitialSpins || SLOT_CONFIG.bonusBuy.freeSpins}` : slot.autoRunning ? `АВТО ${slot.autoRunsRemaining}` : "";
...
function continueSlotFlowsAfterRound({ bonusBuy = false, isFreeSpin = false, presentation = null, duration = 0 } = {}) {
  ...
  if (slot.bonusSessionActive && (bonusBuy || isFreeSpin)) {
    const delay = presentation ? Math.min(Math.max(duration + 260, 1450), 4300) : 720;
...
async function playPharaohSlot({ bonusBuy = false, autoStep = false, bonusSessionStep = false } = {}) {
  ...
  try {
    ...
    if (ultraLiteRenderer && !shouldUsePharaohCanvasRenderer()) {
      ...
      const presentationDuration = bonusBuy ? 3600 : 3200;
      ...
      await pharaohCanvasRenderer.playResult(
        ...
        bonusBuy || isFreeSpin ? "free" : "base",
        {
          ...
          onStep: (step) => {
            ...
            if (freeSpins) {
              const freeText = slot.bonusSessionActive ? `FS ${slot.freeSpins}` : slot.freeSpins > 0 ? `FREE SPINS ${slot.freeSpins}` : "FREE SPINS 0";
              const sessionPlayed = slot.bonusSessionActive && step.phase === "free" ? slot.bonusSessionPlayed + step.freeSpinIndex : slot.bonusSessionPlayed;
              const autoText = slot.bonusSessionActive ? `${sessionPlayed}/${slot.bonusSessionInitialSpins || SLOT_CONFIG.bonusBuy.freeSpins}` : slot.autoRunning ? `АВТО ${slot.autoRunsRemaining}` : "";
              freeSpins.textContent = autoText ? `${freeText} · ${autoText}` : freeText;
              freeSpins.classList.toggle("active", slot.freeSpins > 0 || slot.autoRunning || slot.bonusSessionActive);
            }
            renderSlotHistory(slot);
