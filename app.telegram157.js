...
class PharaohCanvasRenderer {
  ...
  async animateCascadeTransition(fromGrid, winningKeys, nextGrid, duration = 560) {
    ...
    await this.animate(duration, (progress) => {
      const vanishProgress = Math.min(1, progress / 0.32);
      const dropProgress = Math.max(0, Math.min(1, (progress - 0.32) / 0.68));
      const incomingProgress = Math.max(0, Math.min(1, (progress - 0.48) / 0.52));
      const dropEase = easeOutBack(dropProgress);
      const incomingEaseBase = easeOut(incomingProgress);
      const bounce = dropProgress > 0.66
        ? Math.sin(Math.min(1, (dropProgress - 0.66) / 0.34) * Math.PI) * Math.max(1.4, cellH * 0.055)
        : 0;
      const shimmer = 0.5 + Math.sin(progress * Math.PI * 5) * 0.5;

      this.drawBackground();
      if (progress < 0.38) {
        if (!this.drawWinGroupNetwork([{ keys: [...keys] }], Math.min(1, progress / 0.38))) {
          this.drawWinLinks(keys, Math.min(1, progress / 0.38));
        }
      }
      ...
      sprites.survivors.forEach((sprite) => {
        ...
        const y = dropProgress <= 0
          ? fromY
          : fromY + (toY - fromY) * dropEase + (sprite.fromRow !== sprite.toRow ? bounce : 0);
        this.drawSymbol(sprite.symbolId, x, y, cellW, cellH, {
          alpha: 0.9 + dropProgress * 0.1,
          scale: 0.99 + dropProgress * 0.01,
        });
      });
      ...
      sprites.vanishing.forEach((sprite) => {
        ...
        const alpha = Math.max(0, 1 - vanishProgress);
        const scale = 1 + Math.sin(vanishProgress * Math.PI) * 0.13 + vanishProgress * 0.08;
        this.drawSymbol(sprite.symbolId, x, y, cellW, cellH, {
          winning: true,
          alpha,
          scale,
      ...
      if (progress < 0.55) this.drawWinBurst(keys, Math.min(1, progress / 0.55));
    });
  ...
  try {
    ...
    if (canvasRenderer) {
      ...
      await pharaohCanvasRenderer.playResult(
        ...
        {
          ...(ultraLiteRenderer
            ? { reel: 820, drop: 220, win: 640, cascade: 1080, empty: 70 }
            : liteRenderer ? { reel: 920, drop: 280, win: 780, cascade: 1120, empty: 90 } : { reel: 980, win: 860, cascade: 1180 }),
          onStep: (step) => {
            if (slot.spinRunId !== runId) return;
