...
const SLOT_CONFIG = {
  ...
  symbols: [
    ...
    { id: "multi", label: "×", name: "Множитель", tier: "multi", weight: 0.55, color: "#ff73d5", icon: "./assets/pharaoh-icons-premium/multiplier-orb-v2.png" },
  ],
  payouts: {
...
const STARTUP_ASSETS = [
  ...
  "./assets/pharaoh-icons-premium/multiplier-orb-v2.png",
...
const SLOT_WIN_TREASURE_PARTICLES = {
  ...
  orb: "./assets/pharaoh-icons-premium/multiplier-orb-v2.png",
};
...
class PharaohCanvasRenderer {
  ...
  drawSymbol(symbolId, x, y, w, h, options = {}) {
    ...
    const isScatter = symbol.id === "scatter";
    const isFeature = symbol.tier === "scatter" || symbol.tier === "multi";
    const iconRatio = symbol.tier === "scatter" ? 0.88 : symbol.tier === "multi" ? 0.78 : symbol.tier === "high" ? 0.84 : 0.82;
    ...
    if (symbol.id === "multi" && multiplierValue > 0) {
      ...
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      const badgeW = Math.max(iconSize * 0.62, ctx.measureText(label).width + 8);
      const badgeH = fontSize + 6;
      const badgeX = iconCx - badgeW / 2;
      const badgeY = iconCy - badgeH / 2 + iconSize * 0.08;
      ctx.fillStyle = "rgba(28, 7, 43, 0.9)";
      ...
      ctx.fillText(label, iconCx, badgeY + badgeH / 2 + 0.5);
    }
    ctx.restore();
  }
...
async function playPharaohSlot({ bonusBuy = false, autoStep = false, bonusSessionStep = false } = {}) {
  ...
  try {
    ...
    window.mirageAudio?.pharaohSpin(bonusBuy || isFreeSpin);
    renderPharaoh();
