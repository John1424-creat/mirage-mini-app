...
function positionSlotPresentationPanel(panel) {
  ...
  const top = gridRect.top + gridRect.height * 0.24;
  const left = gridRect.left + gridRect.width / 2;
  const width = Math.min(gridRect.width + 2, viewportWidth - 56);
...
function startSlotWinCeremony(type, amount, preferredDuration = 3000) {
  ...
  const draw = (now) => {
    ...
    if (alpha > 0) {
      ...
      wash.addColorStop(0, `rgba(255, 196, 91, ${0.06 * alpha})`);
      wash.addColorStop(0.52, `rgba(184, 38, 180, ${0.055 * alpha})`);
