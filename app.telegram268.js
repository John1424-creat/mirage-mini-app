const state = {
  ...
  carpet: {
    ...
    visualRotate: -8,
    visualProgress: 0,
    crashAnimationStartedAt: null,
    crashAnimationProgress: 0,
    crashOriginX: 0,
    crashOriginY: 0,
    crashOriginScale: 1,
    crashOriginRotate: -8,
    crashOriginProgress: 0,
    crashTargetX: 0,
    crashTargetY: 0,
    crashTargetScale: 1,
    crashTargetRotate: -8,
    cashoutAnimationStartedAt: null,
    cashoutAnimationProgress: 0,
    cashoutConfirmProgress: 0,
    cashoutOriginProgress: 0,
    cashoutOriginScale: 1,
  ...
  slot: {
    ...
    history: [],
    explanation: null,
    presentation: { type: "idle", title: "", value: "", note: "", win: 0, displayWin: 0, ruby: false },
    ...
    bonusSessionRunning: false,
    bonusSessionInitialSpins: 0,
    bonusSessionPlayed: 0,
    bonusSessionTotalWin: 0,
    bonusSessionFinalizing: false,
    bonusSessionFinalTimer: null,
    freeSpinMultiplierBank: 0,
    multiplierMeterPrevious: 1,
    multiplierMeterDelta: 0,
    multiplierMeterPreview: 1,
    multiplierMeterFormula: "",
    multiplierMeterMode: "bonus",
    multiplierMeterPulseTimer: null,
    bonusSessionTimer: null,
  },
  tier: "cheap",
  balls: 10,
  ledger: [],
...
const CARPET_ACCELERATION_MS = 1200;
const CARPET_FAST_PATH_SHARE = 0.5;
const CARPET_FAST_MOTION_TAU_MS = 200;
const CARPET_SLOW_MOTION_TAU_MS = 5500;
const CARPET_HERO_BASE_X = 28;
const CARPET_HERO_BASE_Y = 282;
const CARPET_HERO_WIDTH = 102;
const CARPET_HERO_HEIGHT = 153;
const CARPET_HERO_ORIGIN_X = CARPET_HERO_WIDTH * 0.56;
const CARPET_HERO_ORIGIN_Y = CARPET_HERO_HEIGHT * 0.62;
const CARPET_ENGINE_X = CARPET_HERO_WIDTH * 0.18;
const CARPET_ENGINE_Y = CARPET_HERO_HEIGHT * 0.73;
const CARPET_PATH_START_X = 49.1;
const CARPET_PATH_START_Y = 398.9;
const CARPET_PATH_END_X = 230.1;
const CARPET_PATH_END_Y = 164.7;
const CARPET_CRASH_IMPACT_MS = 110;
const CARPET_CRASH_DISSOLVE_MS = 620;
const CARPET_CRASH_RESULT_HOLD_MS = 650;
const CARPET_CASHOUT_CONFIRM_MS = 120;
const CARPET_CASHOUT_EXIT_MS = 670;
const CARPET_CASHOUT_RESULT_HOLD_MS = 720;
const CARPET_CASHOUT_MIN_EXIT_PROGRESS = 0.97;
const CARPET_DISSOLVE_FRAME_COUNT = 30;
...
const carpetMotionGeometry = {
  element: null,
  length: 0,
  startPoint: null,
  startHeading: -Math.PI / 2,
  endHeading: -Math.PI / 8,
};
const carpetHeroGeometry = {
  element: null,
  baseX: CARPET_HERO_BASE_X,
  baseY: CARPET_HERO_BASE_Y,
  originX: CARPET_HERO_ORIGIN_X,
  originY: CARPET_HERO_ORIGIN_Y,
  engineX: CARPET_ENGINE_X,
  engineY: CARPET_ENGINE_Y,
};
const carpetDissolve = {
  frames: [],
  particles: [],
  ready: false,
  preparing: false,
...
const SLOT_CONFIG = {
  ...
  symbols: [
    { id: "pharaoh", label: "♛", name: "Фараон", tier: "high", weight: 4.4, color: "#ffd97a", icon: "./assets/pharaoh-icons-premium/pharaoh-mask-v1.webp" },
    { id: "lamp", label: "✦", name: "Лампа", tier: "high", weight: 5.2, color: "#ffb44d", icon: "./assets/pharaoh-icons-premium/genie-lamp-v1.webp" },
    { id: "ruby", label: "◆", name: "Рубин", tier: "high", weight: 6.4, color: "#ff4fb3", icon: "./assets/pharaoh-icons-premium/ruby-v1.webp" },
    { id: "scarab", label: "✹", name: "Скарабей", tier: "mid", weight: 8.2, color: "#62dfb4", icon: "./assets/pharaoh-icons-premium/scarab-v1.webp" },
    { id: "scroll", label: "▰", name: "Свиток", tier: "mid", weight: 9.4, color: "#f7cf72", icon: "./assets/pharaoh-icons-premium/scroll-v1.webp" },
    { id: "ankh", label: "☥", name: "Анкх", tier: "mid", weight: 10.2, color: "#d7a8ff", icon: "./assets/pharaoh-icons-premium/ankh-v1.webp" },
    { id: "coin", label: "●", name: "Монета", tier: "low", weight: 12.5, color: "#ffdc75", icon: "./assets/pharaoh-icons-premium/coin-v1.webp" },
    { id: "blue", label: "◆", name: "Сапфир", tier: "low", weight: 12.8, color: "#66b8ff", icon: "./assets/pharaoh-icons-premium/sapphire-v1.webp" },
    { id: "green", label: "◆", name: "Изумруд", tier: "low", weight: 13.2, color: "#8dff9d", icon: "./assets/pharaoh-icons-premium/emerald-v1.webp" },
    { id: "scatter", label: "☀", name: "Дар джина", tier: "scatter", weight: 2.4, color: "#f4ccff", icon: "./assets/pharaoh-icons-premium/genie-scatter-v1.webp" },
    { id: "multi", label: "×", name: "Множитель", tier: "multi", weight: 0.55, color: "#ff73d5", icon: "./assets/pharaoh-icons-premium/multiplier-lamp-v1.webp" },
  ],
  payouts: {
...
const STARTUP_ASSETS = [
  ...
  "./assets/plinko-risk-high-v2.webp",
  "./assets/plinko-risk-medium-v2.webp",
  "./assets/plinko-risk-low-v2.webp",
  "./assets/carpet-hero-clean.svg",
  "./assets/plinko-monkey-guardian-v2.webp",
  "./assets/plinko-reward-coin-atlas-v1.png",
  "./assets/pharaoh-icons-premium/pharaoh-mask-v1.webp",
  "./assets/pharaoh-icons-premium/genie-lamp-v1.webp",
  "./assets/pharaoh-icons-premium/ruby-v1.webp",
  "./assets/pharaoh-icons-premium/scarab-v1.webp",
  "./assets/pharaoh-icons-premium/scroll-v1.webp",
  "./assets/pharaoh-icons-premium/ankh-v1.webp",
  "./assets/pharaoh-icons-premium/coin-v1.webp",
  "./assets/pharaoh-icons-premium/sapphire-v1.webp",
  "./assets/pharaoh-icons-premium/emerald-v1.webp",
  "./assets/pharaoh-icons-premium/genie-scatter-v1.webp",
  "./assets/pharaoh-icons-premium/multiplier-lamp-v1.webp",
  "./assets/pharaoh-icons/genie-reaction-full.webp",
  "./assets/pharaoh-icons/genie-state-idle-clean.webp",
  "./assets/pharaoh-icons/genie-state-spin-clean.webp",
  "./assets/pharaoh-icons/genie-state-cascade-clean.webp",
  "./assets/pharaoh-icons/genie-state-win-clean.webp",
  "./assets/pharaoh-icons/genie-state-legend-clean.webp",
  "./assets/pharaoh-genie-premium-bust-v1.png",
  "./assets/pharaoh-genie-premium-idle-smoke-fit-v1.png",
  "./assets/pharaoh-genie-premium-spin-safe-v1.png",
  "./assets/pharaoh-genie-premium-win-safe-v1.png",
  "./assets/pharaoh-genie-premium-legend-safe-v1.png",
  "./assets/pharaoh-icons/pharaoh-win-crest.webp",
];
const STARTUP_MIN_VISIBLE_MS = 900;
const STARTUP_MAX_WAIT_MS = 5200;

const SLOT_GENIE_REACTION_ASSETS = {
  idle: "./assets/pharaoh-genie-pose-idle-v2.png",
  casting: "./assets/pharaoh-genie-pose-spin-v2.png",
  spin: "./assets/pharaoh-genie-pose-spin-v2.png",
  cascade: "./assets/pharaoh-genie-pose-win-v2.png",
  win: "./assets/pharaoh-genie-pose-win-v2.png",
  big: "./assets/pharaoh-genie-pose-legend-v2.png",
  mega: "./assets/pharaoh-genie-pose-legend-v2.png",
  legend: "./assets/pharaoh-genie-pose-legend-v2.png",
  free: "./assets/pharaoh-genie-pose-win-v2.png",
};

const SLOT_GENIE_DEBUG_STATES = new Set(Object.keys(SLOT_GENIE_REACTION_ASSETS));
const SLOT_GENIE_DEBUG_TEXT = {
  idle: "ДАР ДЖИНА",
  casting: "МАГИЯ ЛАМПЫ",
  spin: "МАГИЯ ЛАМПЫ",
  cascade: "ПОТОК СОКРОВИЩ",
  win: "ДАР ДЖИНА",
  big: "ЗОЛОТО ФАРАОНА",
  mega: "СОКРОВИЩЕ ЛАМПЫ",
  legend: "ВОЛЯ ФАРАОНА",
  free: "ВРЕМЯ ЧУДЕС",
};

const SLOT_WIN_CEREMONY_ASSET = "./assets/pharaoh-icons/pharaoh-win-crest.webp";
const SLOT_WIN_CEREMONY_TIERS = new Set(["big", "mega", "legend", "free-win"]);
const SLOT_WIN_CEREMONY_LABELS = {
  big: "ДАР ЛАМПЫ",
  mega: "СОКРОВИЩЕ",
  legend: "ВОЛЯ ФАРАОНА",
  "free-win": "FREE SPINS",
};
const SLOT_WIN_TREASURE_PARTICLES = {
  coin: "./assets/pharaoh-icons-premium/coin-v1.webp",
  ruby: "./assets/pharaoh-icons-premium/ruby-v1.webp",
  blueGem: "./assets/pharaoh-icons-premium/sapphire-v1.webp",
  greenGem: "./assets/pharaoh-icons-premium/emerald-v1.webp",
  scarab: "./assets/pharaoh-icons-premium/scarab-v1.webp",
  sun: "./assets/pharaoh-icons-premium/genie-scatter-v1.webp",
  scroll: "./assets/pharaoh-icons-premium/scroll-v1.webp",
  ankh: "./assets/pharaoh-icons-premium/ankh-v1.webp",
  orb: "./assets/pharaoh-icons-premium/multiplier-lamp-v1.webp",
};
const slotWinTreasureImageCache = new Map();
const slotWinTreasureSpriteCache = new Map();
const SLOT_WIN_TREASURE_SPRITE_SIZE = 128;
...
function formatHeaderBalance(value) {
  ...
  if (rounded < 10_000_000) return format(rounded);
...
function formatSlotSpinCount(count) {
  const amount = Math.max(0, Math.round(count));
  return `${amount} ${amount === 1 ? "СПИН" : "СПИНОВ"}`;
}
...
function getSlotFreeSpinMultiplierValue() {
  const table = SLOT_CONFIG.freeSpinMultiplier.values;
...
function getSlotBaseMultiplierValue() {
  return 2 + Math.floor(Math.random() * 4);
}

function getSlotCellSymbolId(cellValue) {
  return String(cellValue || "").split(":", 1)[0];
}

function getSlotCellMultiplierValue(cellValue) {
  if (getSlotCellSymbolId(cellValue) !== "multi") return 0;
  const value = Number(String(cellValue).split(":")[1]);
  return Number.isFinite(value) ? value : 0;
}

function createSlotCell(context = "base") {
  const symbol = getSlotWeightedSymbol(context);
  if (symbol.id !== "multi") return symbol.id;
  const value = context === "free" ? getSlotFreeSpinMultiplierValue() : getSlotBaseMultiplierValue();
  return `multi:${value}`;
}

function getSlotBonusBuyLimit() {
  return Math.floor(
    SLOT_CONFIG.roomPoolUsd * ECONOMY_CONFIG.rubiesPerUsdt * SLOT_CONFIG.bonusBuy.maxPoolShare
  );
}

function getSlotSymbol(id) {
  const symbolId = getSlotCellSymbolId(id);
...
function createSlotGrid(context = "base") {
  return Array.from({ length: SLOT_CONFIG.gridRows }, () =>
    Array.from({ length: SLOT_CONFIG.gridColumns }, () => createSlotCell(context))
  );
}
...
function getSlotPayoutMultiplier(symbolId, count, payoutScale = SLOT_CONFIG.payoutScale) {
  ...
  return multiplier * payoutScale;
}

function evaluateSlotGrid(grid, payoutScale = SLOT_CONFIG.payoutScale) {
  ...
  grid.forEach((row, rowIndex) => {
    row.forEach((cellValue, colIndex) => {
      const symbolId = getSlotCellSymbolId(cellValue);
      if (symbolId === "scatter") {
        scatterCount += 1;
        return;
      }
      if (symbolId === "multi") {
        multiplierCount += 1;
        multiplierValues.push({ key: `${rowIndex}-${colIndex}`, value: getSlotCellMultiplierValue(cellValue) });
        return;
      }
  ...
  counts.forEach((count, symbolId) => {
    ...
    const multiplier = getSlotPayoutMultiplier(symbolId, count, payoutScale);
...
function cascadeSlotGrid(grid, winningKeys, context = "base") {
  ...
  for (let col = 0; col < columns; col += 1) {
    ...
    for (let row = rows - 1; row >= 0; row -= 1) {
      next[row][col] = remaining.length > 0 ? remaining.shift() : createSlotCell(context);
    }
  }
...
function createSlotSpinResult(stake, context = "base", { freeMultiplierBank = 0 } = {}) {
  ...
  let cascadeIndex = 0;
  const payoutScale = context === "free" ? SLOT_CONFIG.freeSpinPayoutScale : SLOT_CONFIG.payoutScale;

  while (cascadeIndex < 10) {
    const evaluation = evaluateSlotGrid(grid, payoutScale);
    let multiplierAdded = 0;
    if (context === "free" && evaluation.multiplierCount > 0 && evaluation.wins.length > 0) {
      multiplierAdded = evaluation.multiplierValues.reduce((sum, item) => sum + item.value, 0);
      freeMultiplierBank += multiplierAdded;
      freeMultiplierBank = Math.min(freeMultiplierBank, SLOT_CONFIG.freeSpinMultiplier.maxBank);
    }
    const winningKeys = new Set(evaluation.wins.flatMap((win) => win.keys));
    if (evaluation.wins.length > 0) {
      evaluation.multiplierValues.forEach((item) => winningKeys.add(item.key));
    }
    const cascadeMultiplier = evaluation.wins.reduce((sum, win) => sum + win.multiplier, 0);
    const randomMultiplier = evaluation.wins.length > 0 && evaluation.multiplierCount > 0 && context !== "free"
      ? evaluation.multiplierValues.reduce((sum, item) => sum + item.value, 0)
      : 0;
    const appliedMultiplier = context === "free" && evaluation.wins.length > 0
      ? Math.max(1, freeMultiplierBank)
      : randomMultiplier > 0 ? randomMultiplier : 1;
    const baseWin = stake * cascadeMultiplier;
    const cascadeWin = Math.round(baseWin * appliedMultiplier);

    if (cascadeIndex === 0 && evaluation.scatterCount >= 4) {
      freeSpinsAwarded = evaluation.scatterCount >= 6 ? 15 : evaluation.scatterCount === 5 ? 12 : 10;
    }

    const nextGrid = evaluation.wins.length > 0
      ? cascadeSlotGrid(grid, winningKeys, context)
      : null;

    steps.push({
      grid: grid.map((row) => row.slice()),
      nextGrid: nextGrid ? nextGrid.map((row) => row.slice()) : null,
      wins: evaluation.wins,
      winningKeys,
      multiplier: appliedMultiplier,
      multiplierAdded,
      multiplierBank: freeMultiplierBank,
      multiplierValues: evaluation.multiplierValues,
      baseWin,
      win: cascadeWin,
    });
  ...
  return { steps, totalWin, totalMultiplier, freeSpinsAwarded, freeMultiplierBank };
}

function createSlotRoundResult(stake, { bonusBuy = false, freeSpinOnly = false, freeMultiplierBank = 0 } = {}) {
  const steps = [];
  let totalWin = 0;
  ...
  if (bonusBuy) {
    return {
      ...
      retriggers,
      bonusGranted: true,
      freeMultiplierBank: 0,
    };
  }

  const spin = createSlotSpinResult(stake, freeSpinOnly ? "free" : "base", { freeMultiplierBank });
  freeMultiplierBank = spin.freeMultiplierBank || freeMultiplierBank;
  if (freeSpinOnly) {
    freeSpinsPlayed += 1;
    addSpin(spin, "free", freeSpinsPlayed);
    if (spin.freeSpinsAwarded > 0) retriggers += 1;
  } else {
    addSpin(spin, "base");
  }
...
function createSlotAuditRng(seed = 260) {
  let value = seed >>> 0;
  return () => {
    value += 0x6d2b79f5;
    let result = value;
...
function simulateSlotBonusSession(initialSpins, stake) {
  ...
  while (remaining > 0 && played < SLOT_CONFIG.maxBonusSpinsPerRound) {
    remaining -= 1;
    const spin = createSlotSpinResult(stake, "free", { freeMultiplierBank: multiplierBank });
    played += 1;
    totalWin += spin.totalWin;
    multiplierBank = spin.freeMultiplierBank || multiplierBank;

    if (spin.freeSpinsAwarded > 0) {
      const capacity = SLOT_CONFIG.maxBonusSpinsPerRound - played - remaining;
      const awarded = Math.max(0, Math.min(spin.freeSpinsAwarded, capacity));
      if (awarded > 0) {
        remaining += awarded;
        retriggers += 1;
      }
  ...
  return { totalWin, played, multiplierBank, retriggers };
}

function runPharaohEconomyAudit({ baseRounds = 30000, bonusBuys = 10000, stake = 10, seed = 260 } = {}) {
  const originalRandom = Math.random;
  Math.random = createSlotAuditRng(seed);

  try {
    let baseWin = 0;
    let triggeredBonusWin = 0;
    let triggerCount = 0;
    let triggerSpins = 0;
    ...
    for (let index = 0; index < baseRounds; index += 1) {
      ...
      if (base.freeSpinsAwarded > 0) {
        triggerCount += 1;
        const bonus = simulateSlotBonusSession(base.freeSpinsAwarded, stake);
        triggeredBonusWin += bonus.totalWin;
        triggerSpins += bonus.played;
      }
    }

    let boughtBonusWin = 0;
    let boughtBonusSpins = 0;
    let boughtBonusRetriggers = 0;
    let maxBankHits = 0;
    const boughtBonusReturns = [];

    for (let index = 0; index < bonusBuys; index += 1) {
      const bonus = simulateSlotBonusSession(SLOT_CONFIG.bonusBuy.freeSpins, stake);
      boughtBonusWin += bonus.totalWin;
      boughtBonusSpins += bonus.played;
      boughtBonusRetriggers += bonus.retriggers;
      if (bonus.multiplierBank >= SLOT_CONFIG.freeSpinMultiplier.maxBank) maxBankHits += 1;
      boughtBonusReturns.push(bonus.totalWin);
    }

    boughtBonusReturns.sort((a, b) => a - b);
    const quantile = (ratio) => boughtBonusReturns[Math.min(
      boughtBonusReturns.length - 1,
      Math.floor((boughtBonusReturns.length - 1) * ratio)
    )] || 0;
    const baseCost = baseRounds * stake;
    const buyCost = bonusBuys * stake * SLOT_CONFIG.bonusBuy.costMultiplier;

    return {
      seed,
      stake,
      config: {
        targetRtp: SLOT_CONFIG.targetRtp,
        basePayoutScale: SLOT_CONFIG.payoutScale,
        freeSpinPayoutScale: SLOT_CONFIG.freeSpinPayoutScale,
        scatterWeight: SLOT_CONFIG.symbols.find((symbol) => symbol.id === "scatter")?.weight || 0,
        multiplierWeight: SLOT_CONFIG.symbols.find((symbol) => symbol.id === "multi")?.weight || 0,
        bonusBuyCostMultiplier: SLOT_CONFIG.bonusBuy.costMultiplier,
        bonusBuySpins: SLOT_CONFIG.bonusBuy.freeSpins,
        rubiesPerUsdt: ECONOMY_CONFIG.rubiesPerUsdt,
        roomPoolUsd: SLOT_CONFIG.roomPoolUsd,
        bonusBuyLimitRubies: getSlotBonusBuyLimit(),
        multiplierBankCap: SLOT_CONFIG.freeSpinMultiplier.maxBank,
      },
      base: {
        rounds: baseRounds,
        directRtp: baseWin / baseCost,
        scatterTriggerRate: triggerCount / baseRounds,
        averageTriggeredSpins: triggerCount > 0 ? triggerSpins / triggerCount : 0,
        freeSpinRtpContribution: triggeredBonusWin / baseCost,
        combinedRtp: (baseWin + triggeredBonusWin) / baseCost,
      },
      bonusBuy: {
        sessions: bonusBuys,
        rtp: boughtBonusWin / buyCost,
        averageWinInStakes: boughtBonusWin / bonusBuys / stake,
        averageSpins: boughtBonusSpins / bonusBuys,
        retriggersPerSession: boughtBonusRetriggers / bonusBuys,
        maxBankRate: maxBankHits / bonusBuys,
        zeroWinRate: boughtBonusReturns.filter((value) => value === 0).length / bonusBuys,
        p50InStakes: quantile(0.5) / stake,
        p75InStakes: quantile(0.75) / stake,
        p90InStakes: quantile(0.9) / stake,
        p95InStakes: quantile(0.95) / stake,
        p99InStakes: quantile(0.99) / stake,
        maxInStakes: (boughtBonusReturns[boughtBonusReturns.length - 1] || 0) / stake,
      },
    };
  } finally {
...
function initPharaohEconomyAudit() {
  const params = new URLSearchParams(window.location.search);
  if (params.get("debugEconomyAudit") !== "1") return;
  const readCount = (name, fallback, maximum) => Math.min(
    maximum,
    Math.max(1, Number.parseInt(params.get(name) || "", 10) || fallback)
  );
  const result = runPharaohEconomyAudit({
    baseRounds: readCount("auditBaseRounds", 30000, 200000),
    bonusBuys: readCount("auditBonusBuys", 10000, 100000),
    stake: readCount("auditStake", 10, 500),
    seed: readCount("auditSeed", 260, 2147483647),
  });
  document.body.dataset.pharaohEconomyAudit = JSON.stringify(result);
}

function createSlotSymbolIcon(symbol, className = "slot-symbol-orb", multiplierValue = 0) {
  ...
  if (symbol.id === "multi" && multiplierValue > 0) {
    const value = document.createElement("b");
    value.className = "slot-multiplier-value";
    value.textContent = `x${multiplierValue}`;
...
class PharaohCanvasRenderer {
  ...
  drawCellFrame(x, y, w, h, symbol, options = {}) {
    ...
    outer.addColorStop(0, isScatter ? "rgba(255, 245, 184, 0.98)" : isPremium ? "rgba(255, 232, 158, 0.94)" : "rgba(255, 211, 134, 0.72)");
    outer.addColorStop(0.46, isScatter ? "rgba(189, 75, 255, 0.86)" : isFeature ? "rgba(255, 103, 207, 0.78)" : "rgba(178, 83, 153, 0.5)");
    outer.addColorStop(1, "rgba(58, 42, 103, 0.56)");
    const fill = ctx.createRadialGradient(cx, cy - sh * 0.16, sw * 0.08, cx, cy + sh * 0.2, sw * 0.72);
    fill.addColorStop(0, isScatter ? "rgba(255, 226, 166, 0.38)" : isFeature ? "rgba(255, 214, 134, 0.3)" : "rgba(255, 225, 154, 0.16)");
    ...
    if (isScatter) {
      ctx.globalCompositeOperation = "lighter";
      ctx.shadowColor = "rgba(199, 78, 255, 0.86)";
      ctx.shadowBlur = 13 + shimmer * 7;
      ctx.fillStyle = "rgba(137, 45, 200, 0.22)";
      this.roundRect(sx - 1, sy - 1, sw + 2, sh + 2, radius + 2);
      ctx.fill();
      ctx.globalCompositeOperation = "source-over";
    }
    ...
    ctx.strokeStyle = winning ? "rgba(255, 238, 172, 0.98)" : isScatter ? "rgba(255, 238, 172, 0.88)" : isPremium ? "rgba(255, 218, 134, 0.7)" : "rgba(255, 204, 126, 0.44)";
  ...
  drawSymbol(symbolId, x, y, w, h, options = {}) {
    ...
    const isScatter = symbol.id === "scatter";
    const isFeature = symbol.tier === "scatter" || symbol.tier === "multi";
    const iconRatio = symbol.tier === "scatter" ? 0.88 : symbol.tier === "multi" ? 0.84 : symbol.tier === "high" ? 0.84 : 0.82;
    ...
    ctx.beginPath();
    ctx.ellipse(iconCx, iconCy + iconSize * 0.36, iconSize * 0.32, iconSize * 0.08, 0, 0, Math.PI * 2);
    ctx.fill();

    ctx.shadowColor = winning ? "rgba(255, 239, 169, 0.96)" : isScatter ? "rgba(203, 91, 255, 0.92)" : symbol.color;
    ctx.shadowBlur = winning ? 16 : isScatter ? 13 : isFeature ? 7 : 4;
    if (img) {
    ...
    const multiplierValue = getSlotCellMultiplierValue(symbolId);
    if (symbol.id === "multi" && multiplierValue > 0) {
      const label = `x${multiplierValue}`;
      const fontSize = Math.max(8, Math.round(iconSize * 0.23));
      ctx.shadowBlur = 0;
      ctx.font = `900 ${fontSize}px Inter, sans-serif`;
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      const badgeW = Math.max(iconSize * 0.48, ctx.measureText(label).width + 6);
      const badgeH = fontSize + 4;
      const badgeCenterX = iconCx + iconSize * 0.17;
      const badgeCenterY = iconCy + iconSize * 0.28;
      const badgeX = badgeCenterX - badgeW / 2;
      const badgeY = badgeCenterY - badgeH / 2;
      ctx.fillStyle = "rgba(28, 7, 43, 0.94)";
      this.roundRect(badgeX, badgeY, badgeW, badgeH, badgeH / 2);
      ctx.fill();
      ctx.strokeStyle = "rgba(255, 226, 137, 0.94)";
      ctx.lineWidth = 1;
      ctx.stroke();
      ctx.fillStyle = "#fff3be";
      ctx.fillText(label, badgeCenterX, badgeCenterY + 0.5);
    }
    ctx.restore();
  }
  ...
  async animateReel(targetGrid, context = "base", duration = 760, events = {}) {
    this.preload();
    const { pad, gap, cellW, cellH, cols, rows } = this.getLayout();
    const rowStride = cellH + gap;
    const outgoingGrid = this.lastGrid?.length ? this.lastGrid.map((row) => row.slice()) : null;
    const incoming = [];
    for (let col = 0; col < cols; col += 1) {
      for (let row = 0; row < rows; row += 1) {
        incoming.push({
          symbolId: targetGrid[row][col],
          row,
          col,
          delay: col * 0.075 + row * 0.018,
          distance: rowStride * (rows + 1.2 + row * 0.08 + (col % 2) * 0.08),
        });
      }
    }
    const outgoing = [];
    if (outgoingGrid?.length) {
      for (let col = 0; col < cols; col += 1) {
        for (let row = rows - 1; row >= 0; row -= 1) {
          const symbolId = outgoingGrid[row]?.[col];
          if (!symbolId) continue;
          outgoing.push({
            symbolId,
            row,
            col,
            delay: col * 0.052 + (rows - 1 - row) * 0.014,
            distance: rowStride * (rows + 1.36 + (rows - row) * 0.07 + (col % 2) * 0.06),
          });
        }
      }
    }
    ...
    const settledColumns = new Set();

    await this.animate(duration, (progress) => {
      this.drawBackground();
      const ctx = this.ctx;
      if (outgoing.length && progress < 0.72) {
        outgoing.forEach((sprite) => {
          const local = Math.max(0, Math.min(1, (progress - sprite.delay) / 0.4));
          if (local <= 0) {
            const x = pad + sprite.col * (cellW + gap);
            const y = pad + sprite.row * rowStride;
            this.drawSymbol(sprite.symbolId, x, y, cellW, cellH, { alpha: 0.94, scale: 0.99 });
            return;
          }
          if (local >= 1) return;
          const x = pad + sprite.col * (cellW + gap);
          const y = pad + sprite.row * rowStride + sprite.distance * Math.pow(local, 2.1);
          const alpha = Math.max(0, 1 - local * 0.72);
          const scale = 1 - local * 0.08;
          if (local < 0.86) {
            ctx.save();
            ctx.globalCompositeOperation = "lighter";
            const trail = ctx.createLinearGradient(x, y - rowStride * 0.42, x, y + rowStride * 0.76);
            trail.addColorStop(0, "rgba(255, 226, 142, 0.14)");
            trail.addColorStop(0.5, "rgba(255, 88, 207, 0.12)");
            trail.addColorStop(1, "rgba(255, 226, 142, 0)");
            ctx.fillStyle = trail;
            this.roundRect(x + cellW * 0.18, y - rowStride * 0.34, cellW * 0.64, rowStride * 0.9, Math.max(5, cellW * 0.16));
            ctx.fill();
            ctx.restore();
          }
          this.drawSymbol(sprite.symbolId, x, y, cellW, cellH, { alpha, scale });
        });
      }

      incoming.forEach((sprite) => {
        const entryStart = outgoing.length ? 0.29 : 0.02;
        const delay = entryStart + sprite.delay * (outgoing.length ? 0.72 : 1);
        const local = Math.max(0, Math.min(1, (progress - delay) / Math.max(0.22, 1 - delay)));
        if (local <= 0) return;
        const targetX = pad + sprite.col * (cellW + gap);
        const targetY = pad + sprite.row * rowStride;
        const fall = easeOutBack(Math.min(1, local));
        const settle = easeOutCubic(Math.min(1, local));
        const y = targetY - sprite.distance * (1 - fall);
        const alpha = Math.min(1, 0.16 + settle * 0.92);
        const scale = 0.9 + Math.min(1, local) * 0.1;
        if (local < 0.86) {
          ctx.save();
          ctx.globalCompositeOperation = "lighter";
          const streakLength = Math.min(rowStride * 0.92, sprite.distance * (1 - settle) * 0.16 + 10);
          const gradient = ctx.createLinearGradient(targetX, y - streakLength, targetX, y + cellH * 0.45);
          gradient.addColorStop(0, "rgba(255, 224, 130, 0)");
          gradient.addColorStop(0.55, "rgba(255, 112, 209, 0.16)");
          gradient.addColorStop(1, "rgba(255, 232, 156, 0.22)");
          ctx.fillStyle = gradient;
          this.roundRect(targetX + cellW * 0.17, y - streakLength, cellW * 0.66, streakLength + cellH * 0.54, Math.max(5, cellW * 0.18));
          ctx.fill();
          ctx.restore();
        }
        const bounce = local > 0.72 ? Math.sin(Math.min(1, (local - 0.72) / 0.28) * Math.PI) * Math.max(1.2, cellH * 0.045) : 0;
        this.drawSymbol(sprite.symbolId, targetX, y + bounce, cellW, cellH, { alpha, scale });
      });

      if (progress > 0.86) {
        const glowProgress = Math.min(1, (progress - 0.86) / 0.14);
        ctx.save();
        ctx.globalCompositeOperation = "lighter";
        const glow = ctx.createLinearGradient(pad, pad, pad + cols * (cellW + gap), pad + rows * rowStride);
        glow.addColorStop(0, `rgba(255, 227, 138, ${0.1 * glowProgress})`);
        glow.addColorStop(0.5, `rgba(255, 91, 207, ${0.08 * glowProgress})`);
        glow.addColorStop(1, "rgba(255, 227, 138, 0)");
        ctx.strokeStyle = glow;
        ctx.lineWidth = 1.4;
        this.roundRect(pad - 2, pad - 2, this.width - pad * 2 + 4, this.height - pad * 2 + 4, 12);
        ctx.stroke();
        ctx.restore();
      }

      for (let col = 0; col < cols; col += 1) {
        const settleAt = 0.82 + col * 0.027;
        if (progress >= settleAt && !settledColumns.has(col)) {
          settledColumns.add(col);
          if (typeof events.onReelStop === "function") events.onReelStop(col);
        }
      }
    });
    this.drawGrid(targetGrid);
    if (typeof events.onReelSettled === "function") events.onReelSettled();
  }
  ...
  getCascadeSprites(fromGrid, winningKeys, nextGrid) {
    ...
    for (let col = 0; col < cols; col += 1) {
      const columnSurvivors = [];
      for (let row = rows - 1; row >= 0; row -= 1) {
        const symbolId = fromGrid[row]?.[col];
        if (!symbolId) continue;
        if (keys.has(`${row}-${col}`)) {
          vanishing.push({ symbolId, row, col });
        } else {
          columnSurvivors.push({ symbolId, fromRow: row, col });
        }
      }

      columnSurvivors.forEach((sprite, index) => {
        survivors.push({
          ...sprite,
          toRow: rows - 1 - index,
        });
      ...
      const newCount = rows - columnSurvivors.length;
      for (let row = 0; row < newCount; row += 1) {
        incoming.push({
          symbolId: nextGrid[row]?.[col],
          fromRow: row - newCount - 1.15,
          toRow: row,
          col,
        });
      }
    ...
    return { survivors, incoming, vanishing };
  }

  async animateCascadeTransition(fromGrid, winningKeys, nextGrid, duration = 560) {
    await this.preload();
    const keys = winningKeys || new Set();
    if (!fromGrid?.length || !nextGrid?.length || !keys.size) {
      if (nextGrid?.length) this.drawGrid(nextGrid);
      return;
    }

    const sprites = this.getCascadeSprites(fromGrid, keys, nextGrid);
    const { pad, gap, cellW, cellH } = this.getLayout();
    const rowStride = cellH + gap;
    ...
    const vanishDuration = Math.max(230, Math.min(360, duration * 0.34));
    const dropDuration = Math.max(420, duration - vanishDuration);

    await this.animate(vanishDuration, (progress) => {
      const shimmer = 0.5 + Math.sin(progress * Math.PI * 4) * 0.5;
      this.drawBackground();
      if (!this.drawWinGroupNetwork([{ keys: [...keys] }], progress)) {
        this.drawWinLinks(keys, progress);
      }

      sprites.survivors.forEach((sprite) => {
        const x = pad + sprite.col * (cellW + gap);
        const y = pad + sprite.fromRow * rowStride;
        this.drawSymbol(sprite.symbolId, x, y, cellW, cellH, {
          alpha: 0.94,
          scale: 1,
        });
      });

      sprites.vanishing.forEach((sprite) => {
        const x = pad + sprite.col * (cellW + gap);
        const y = pad + sprite.row * rowStride;
        const fade = progress < 0.42 ? 0 : Math.min(1, (progress - 0.42) / 0.58);
        const alpha = Math.max(0, 1 - fade);
        const scale = 1 + Math.sin(progress * Math.PI) * 0.16 + fade * 0.1;
        this.drawSymbol(sprite.symbolId, x, y, cellW, cellH, {
          winning: true,
          alpha,
          scale,
    ...
    await this.animate(dropDuration, (progress) => {
      const survivorProgress = Math.min(1, progress / 0.86);
      const incomingProgress = Math.max(0, Math.min(1, (progress - 0.18) / 0.82));
      const dropEase = easeOutBack(survivorProgress);
      const survivorBounce = survivorProgress > 0.66
        ? Math.sin(Math.min(1, (survivorProgress - 0.66) / 0.34) * Math.PI) * Math.max(1.4, cellH * 0.055)
        : 0;
      ...
      sprites.survivors.forEach((sprite) => {
        const x = pad + sprite.col * (cellW + gap);
        const fromY = pad + sprite.fromRow * rowStride;
        const toY = pad + sprite.toRow * rowStride;
        const y = fromY + (toY - fromY) * dropEase + (sprite.fromRow !== sprite.toRow ? survivorBounce : 0);
        this.drawSymbol(sprite.symbolId, x, y, cellW, cellH, {
          alpha: 0.9 + survivorProgress * 0.1,
          scale: 0.99 + survivorProgress * 0.01,
        });
      });

      sprites.incoming.forEach((sprite, index) => {
        const columnDelay = sprite.col * 0.035 + index * 0.012;
        const local = Math.max(0, Math.min(1, (incomingProgress - columnDelay) / Math.max(0.42, 1 - columnDelay)));
        if (local <= 0) return;
        const incomingEase = easeOut(local);
        const x = pad + sprite.col * (cellW + gap);
        const fromY = pad + sprite.fromRow * rowStride;
        const toY = pad + sprite.toRow * rowStride;
        const y = fromY + (toY - fromY) * (1 - Math.pow(1 - incomingEase, 2.6));
        if (local < 0.82) {
          const ctx = this.ctx;
          ctx.save();
          ctx.globalCompositeOperation = "lighter";
          const trail = ctx.createLinearGradient(x, y - rowStride * 0.65, x, y + cellH * 0.48);
          trail.addColorStop(0, "rgba(255, 226, 142, 0)");
          trail.addColorStop(0.58, "rgba(255, 96, 210, 0.12)");
          trail.addColorStop(1, "rgba(255, 231, 154, 0.2)");
          ctx.fillStyle = trail;
          this.roundRect(x + cellW * 0.2, y - rowStride * 0.58, cellW * 0.6, rowStride * 0.78, Math.max(5, cellW * 0.16));
          ctx.fill();
          ctx.restore();
        }
        this.drawSymbol(sprite.symbolId, x, y, cellW, cellH, {
          alpha: 0.25 + incomingEase * 0.75,
          scale: 0.92 + incomingEase * 0.08,
        });
      });
    });

    this.drawGrid(nextGrid);
  }
  ...
  async animateWins(grid, winningKeys, wins = [], duration = 620, emptyDelay = 120) {
    ...
    await this.animate(duration, (progress) => {
      const hold = progress < 0.72 ? Math.min(1, progress / 0.72) : 1;
      const exit = progress > 0.72 ? (progress - 0.72) / 0.28 : 0;
      const pulse = 1 + Math.sin(progress * Math.PI * 5) * 0.035 + Math.sin(hold * Math.PI) * 0.025;
      const shimmer = 0.5 + Math.sin(progress * Math.PI * 7) * 0.5;
      const { pad, gap, cellW, cellH } = this.getLayout();
      this.drawBackground();
      if (!this.drawWinGroupNetwork(wins, hold)) this.drawWinLinks(keys, hold);
      grid.forEach((row, rowIndex) => {
        row.forEach((symbolId, colIndex) => {
          const key = `${rowIndex}-${colIndex}`;
          const x = pad + colIndex * (cellW + gap);
          ...
          const alpha = isWinning && exit > 0 ? 1 - exit * 0.18 : 1;
          this.drawSymbol(symbolId, x, y, cellW, cellH, { winning: isWinning, alpha, scale: isWinning ? pulse : 1, shimmer });
        });
      });
      this.drawWinBurst(keys, hold);
    });
  }
  ...
  async playResult(displaySteps, context = "base", timings = {}) {
    ...
    const cascadeDuration = timings.cascade ?? 560;
    const emptyDelay = timings.empty ?? 120;
    await this.animateReel(displaySteps[0].grid, context, reelDuration, {
      onReelStop: timings.onReelStop,
      onReelSettled: timings.onReelSettled,
    });
    for (const [index, step] of displaySteps.entries()) {
      if (index > 0 && !displaySteps[index - 1]?.nextGrid) await this.animateDrop(step.grid, dropDuration);
      ...
      if (step.nextGrid && step.wins?.length) {
        if (typeof timings.onCascadeDrop === "function") timings.onCascadeDrop(step, index);
        await this.animateCascadeTransition(step.grid, step.winningKeys, step.nextGrid, cascadeDuration);
      }
    }
  }
}
...
function renderSlotPaytable() {
  ...
  [
    {
      id: "scatter",
      title: "Дар джина (Scatter)",
      description: "4 / 5 / 6+ символов открывают 10 / 12 / 15 free spins.",
    },
    {
      id: "multi",
      title: "Множитель",
      description: "Показывает x2 / x3 / x5 / x10, один раз срабатывает на выигрышном каскаде и исчезает; во free spins пополняет общий множитель до x10.",
    },
  ].forEach((feature) => {
    const symbol = getSlotSymbol(feature.id);
    const row = document.createElement("div");
    row.className = "slot-paytable-feature";
    row.appendChild(createSlotSymbolIcon(symbol, "slot-paytable-icon"));
    const copy = document.createElement("span");
    copy.innerHTML = `<b>${feature.title}</b>${feature.description}`;
    row.appendChild(copy);
    special.appendChild(row);
  });
  const purchase = document.createElement("span");
  purchase.className = "slot-paytable-purchase";
  purchase.innerHTML = "<b>Покупка бонуса</b> стоит 10 ставок и сразу открывает 15 free spins.";
  special.appendChild(purchase);
  root.appendChild(special);
  root.dataset.ready = "true";
}

function getSlotVisibleStatusMessage(message) {
  const text = String(message || "");
  return /^Выигрыш\b/.test(text) || /\+\d/.test(text) || text.includes("💎") ? "" : text;
}

function updateSlotStatus(slot = state.slot, message = slot.message) {
  const status = $("#slot-status");
  if (!status) return;
  const visibleMessage = getSlotVisibleStatusMessage(message);
  const bonusActive = slot.bonusSessionActive || slot.bonusSessionFinalizing || slot.freeSpins > 0;
  status.textContent = bonusActive ? "" : visibleMessage;
  status.classList.toggle("slot-status-multiplier", bonusActive);
  status.classList.toggle("slot-status-empty", !status.textContent);
}

function positionSlotMultiplierMeter(meter = $("#slot-multiplier-meter")) {
  const system = $("#slot-multiplier-system");
  const grid = $("#slot-grid");
  if (!meter || !system || !grid) return;
  // Use local layout coordinates so the app-shell mobile scale is applied only once.
  system.style.setProperty("--slot-system-top", `${grid.offsetTop}px`);
  system.style.setProperty("--slot-system-left", `${grid.offsetLeft}px`);
  system.style.setProperty("--slot-system-width", `${grid.offsetWidth}px`);
  system.style.setProperty("--slot-system-height", `${grid.offsetHeight}px`);
}

function renderSlotMultiplierMeter(slot = state.slot) {
  const meter = $("#slot-multiplier-meter");
  const system = $("#slot-multiplier-system");
  if (!meter || !system) return;
  const debug = new URLSearchParams(window.location.search).get("debugMultiplierMeter") === "1";
  const bonusActive = slot.bonusSessionActive || slot.bonusSessionFinalizing || slot.freeSpins > 0;
  const active = debug || bonusActive || meter.classList.contains("transient");
  const cascadeMode = slot.multiplierMeterMode === "cascade" && !bonusActive;
  const value = cascadeMode
    ? Math.max(1, slot.multiplierMeterPreview || 1)
    : Math.max(1, slot.freeSpinMultiplierBank || 0);
  const labelNode = meter.querySelector(".slot-multiplier-meter-label");
  const valueNode = $("#slot-multiplier-meter-value");
  const deltaNode = $("#slot-multiplier-meter-delta");
  const formulaNode = $("#slot-multiplier-meter-formula");
  meter.classList.toggle("show", active);
  meter.classList.toggle("cascade-mode", cascadeMode);
  meter.setAttribute("aria-hidden", active ? "false" : "true");
  system.classList.toggle("show", active);
  system.classList.toggle("collecting", meter.classList.contains("collecting"));
  system.classList.toggle("cascade-mode", cascadeMode);
  system.classList.toggle("multiplier-active", value > 1);
  system.classList.toggle("formula-active", Boolean(slot.multiplierMeterFormula));
  system.setAttribute("aria-hidden", active ? "false" : "true");
  if (labelNode) labelNode.textContent = cascadeMode ? "КАСКАД" : slot.bonusSessionFinalizing ? "ИТОГОВЫЙ" : "МНОЖИТЕЛЬ";
  if (valueNode) valueNode.textContent = `x${value}`;
  if (deltaNode) deltaNode.textContent = slot.multiplierMeterDelta > 0 ? `+x${slot.multiplierMeterDelta}` : "";
  if (formulaNode) formulaNode.textContent = slot.multiplierMeterFormula || "";
  positionSlotMultiplierMeter(meter);
}

function showSlotMultiplierIncrease(previousValue, nextValue, addedValue) {
  const slot = state.slot;
  const meter = $("#slot-multiplier-meter");
  if (!meter || addedValue <= 0 || nextValue <= previousValue) return;
  if (slot.multiplierMeterPulseTimer) window.clearTimeout(slot.multiplierMeterPulseTimer);
  slot.multiplierMeterPrevious = previousValue;
  slot.multiplierMeterDelta = addedValue;
  slot.multiplierMeterMode = "bonus";
  meter.classList.remove("collecting");
  void meter.offsetWidth;
  meter.classList.add("collecting");
  renderSlotMultiplierMeter(slot);
  slot.multiplierMeterPulseTimer = window.setTimeout(() => {
    meter.classList.remove("collecting");
    slot.multiplierMeterDelta = 0;
    slot.multiplierMeterPulseTimer = null;
    renderSlotMultiplierMeter(slot);
  }, 1050);
}

function showSlotMultiplierStep(step, phase = "base") {
  if (!step?.wins?.length || step.win <= 0 || step.multiplier <= 1) return;
  const slot = state.slot;
  const meter = $("#slot-multiplier-meter");
  if (!meter) return;
  const baseWin = step.baseWin ?? (step.win / Math.max(1, step.multiplier));
  slot.multiplierMeterFormula = `${formatSlotFormulaValue(baseWin)} × x${step.multiplier} → ${format(step.win)}`;
  if (phase !== "free") {
    slot.multiplierMeterMode = "cascade";
    slot.multiplierMeterPreview = step.multiplier;
    slot.multiplierMeterDelta = step.multiplierValues?.reduce((sum, item) => sum + item.value, 0) || 0;
    meter.classList.add("transient", "collecting");
  }
  renderSlotMultiplierMeter(slot);
  if (slot.multiplierMeterPulseTimer) window.clearTimeout(slot.multiplierMeterPulseTimer);
  slot.multiplierMeterPulseTimer = window.setTimeout(() => {
    meter.classList.remove("collecting", "transient");
    slot.multiplierMeterDelta = 0;
    slot.multiplierMeterFormula = "";
    slot.multiplierMeterMode = "bonus";
    slot.multiplierMeterPulseTimer = null;
    renderSlotMultiplierMeter(slot);
  }, phase === "free" ? 1450 : 1750);
}

function getSlotReactionContent(slot) {
  if (slot.reaction === "free") return { state: "free", text: "ВРЕМЯ ЧУДЕС" };
  if (slot.status !== "READY") {
    if (slot.winningKeys?.size > 0) return { state: "cascade", text: "ПОТОК СОКРОВИЩ" };
    return { state: "casting", text: "МАГИЯ ЛАМПЫ" };
  }
  if (slot.totalWin > 0) {
    const multiplier = slot.totalWin / Math.max(1, slot.stake);
    if (multiplier >= 100) return { state: "legend", text: "ВОЛЯ ФАРАОНА" };
    if (multiplier >= 30) return { state: "mega", text: "СОКРОВИЩЕ ЛАМПЫ" };
    if (multiplier >= 10) return { state: "big", text: "ЗОЛОТО ФАРАОНА" };
    return { state: "win", text: "ДАР ДЖИНА" };
  }
  if (slot.autoRunning || slot.autoRunsRemaining > 0) return { state: "casting", text: "МАГИЯ ЛАМПЫ" };
  return { state: "idle", text: "ДАР ДЖИНА" };
}

function getSlotSymbolShortName(symbolId) {
  ...
  return names[symbol.id] || symbol.name || symbol.label || symbol.id;
}

function formatSlotMultiplier(value) {
  return `${floorMultiplier(value || 0).toFixed(2)}x`;
}

function formatSlotFormulaValue(value) {
  const rounded = Math.round((Number(value) || 0) * 100) / 100;
  return rounded.toLocaleString("ru-RU", { maximumFractionDigits: 2 });
}

function getPrimarySlotWin(wins = []) {
...
function buildSlotStepExplanation(step) {
  if (!step?.wins?.length || step.win <= 0) return null;
  const primary = getPrimarySlotWin(step.wins);
  if (!primary) return null;
  const symbolName = getSlotSymbolShortName(primary.symbolId);
  const baseMultiplier = formatSlotMultiplier(primary.multiplier);
  const appliedMultiplier = step.multiplier > 1 ? `x${step.multiplier}` : "x1";
  const baseWin = step.baseWin ?? ((step.win || 0) / Math.max(1, step.multiplier || 1));
  const formula = step.multiplier > 1
    ? `${formatSlotFormulaValue(baseWin)} × ${appliedMultiplier} → ${format(step.win)}`
    : `+${format(step.win)}`;
  const cascadeCount = step.wins.length > 1 ? `${step.wins.length} группы` : "1 группа";
  return {
    mode: "explain",
    summary: step.multiplier > 1
      ? `${formatSlotFormulaValue(baseWin)} × ${appliedMultiplier} → ${format(step.win)}`
      : `${primary.count} ${symbolName} • ${baseMultiplier}`,
    items: [
      { text: symbolName, title: getSlotSymbol(primary.symbolId).name },
      { text: `${primary.count} шт`, title: "Количество одинаковых символов" },
      { text: baseMultiplier, title: "Выплата символа" },
      { text: formula, title: "Расчёт выигрыша с множителем" },
    ],
    cascadeCount,
  };
}

function buildSlotResultExplanation(result) {
  const winningSteps = (result?.steps || []).filter((step) => step?.wins?.length && step.win > 0);
  if (!winningSteps.length || result.totalWin <= 0) return null;
  const bestStep = [...winningSteps].sort((a, b) => b.win - a.win)[0];
  const primary = getPrimarySlotWin(bestStep.wins);
  if (!primary) return null;
  const symbolName = getSlotSymbolShortName(primary.symbolId);
  const totalMultiplier = result.totalWin / Math.max(1, state.slot.stake);
  const cascades = winningSteps.length;
  return {
    mode: "explain",
    summary: `${cascades} каск. • ${primary.count} ${symbolName} • ${formatSlotMultiplier(totalMultiplier)}`,
    items: [
      { text: `${cascades} каск`, title: "Количество выигрышных каскадов" },
      { text: symbolName, title: getSlotSymbol(primary.symbolId).name },
      { text: `${primary.count} шт`, title: "Лучший выигрышный набор" },
      { text: formatSlotMultiplier(totalMultiplier), title: "Итоговый множитель от ставки" },
      { text: `+${format(result.totalWin)}`, title: "Итоговый выигрыш" },
    ],
  };
}
...
function ensureSlotPresentationPanel() {
  ...
  const host = document.body || $(".app-shell") || $(".pharaoh-stage");
  if (!host) return null;
  ...
  panel.innerHTML = `
    ...
    <span class="slot-presentation-beam" aria-hidden="true"></span>
    <span class="slot-presentation-sparks" aria-hidden="true"></span>
    <img class="slot-presentation-crest" src="${SLOT_WIN_CEREMONY_ASSET}" alt="" aria-hidden="true" />
    <b class="slot-presentation-title"></b>
    <strong class="slot-presentation-value">
      <span class="slot-presentation-amount"></span>
      <i class="slot-presentation-ruby" aria-hidden="true"></i>
    </strong>
...
function positionSlotPresentationPanel(panel) {
  const gridRoot = $("#slot-grid");
  if (!panel || !gridRoot) return;
  const gridRect = gridRoot.getBoundingClientRect();
  const viewportWidth = window.innerWidth || document.documentElement.clientWidth || gridRect.width;
  if (!viewportWidth || !gridRect.width) return;
  const top = gridRect.top + gridRect.height * 0.5;
  const left = gridRect.left + gridRect.width / 2;
  const width = Math.min(gridRect.width - 54, viewportWidth - 96, 294);
  panel.style.setProperty("--slot-presentation-top", `${Math.round(top)}px`);
  panel.style.setProperty("--slot-presentation-left", `${Math.round(left)}px`);
  panel.style.setProperty("--slot-presentation-width", `${Math.round(width)}px`);
}
...
function renderSlotPresentation(slot) {
  ...
  panel.className = `slot-presentation${visible ? " show" : ""} ${presentation.counting ? " counting" : ""} ${presentation.settled ? " settled" : ""} ${presentation.type || "idle"}`;
  positionSlotPresentationPanel(panel);
  panel.setAttribute("aria-hidden", visible ? "false" : "true");
  if (presentation.durationMs) panel.dataset.durationMs = String(presentation.durationMs);
  else delete panel.dataset.durationMs;
  const title = panel.querySelector(".slot-presentation-title");
  const value = panel.querySelector(".slot-presentation-value");
  const amount = panel.querySelector(".slot-presentation-amount");
  const ruby = panel.querySelector(".slot-presentation-ruby");
  const note = panel.querySelector(".slot-presentation-note");
  if (title) title.textContent = presentation.title || "";
  if (value) value.classList.toggle("has-ruby", Boolean(presentation.ruby));
  if (amount) {
    amount.textContent = presentation.win > 0 ? `+${format(presentation.displayWin || 0)}` : presentation.value || "";
  }
  if (ruby) ruby.style.display = presentation.ruby ? "" : "none";
...
function clearSlotPresentation() {
  ...
  if (slot.presentationFrame) {
    window.cancelAnimationFrame(slot.presentationFrame);
    slot.presentationFrame = null;
  }
  if (slot.prizeFrameTimer) {
    window.clearTimeout(slot.prizeFrameTimer);
    slot.prizeFrameTimer = null;
  }
  const gridRoot = $("#slot-grid");
  if (gridRoot) {
    delete gridRoot.dataset.prizeTier;
    gridRoot.classList.remove("slot-grid-prize-frame");
  }
  slot.presentation = { type: "idle", title: "", value: "", note: "", win: 0, displayWin: 0, ruby: false };
  stopSlotWinCeremony();
  syncNavigationLock();
}

function ensureSlotWinCeremonyOverlay() {
  let overlay = $("#slot-win-ceremony");
  if (overlay) return overlay;
  const host = document.body || $(".app-shell");
  if (!host) return null;
  overlay = document.createElement("div");
  overlay.id = "slot-win-ceremony";
  overlay.className = "slot-win-ceremony slot-win-ceremony-viewport";
  overlay.setAttribute("aria-hidden", "true");
  overlay.innerHTML = `<canvas class="slot-win-ceremony-canvas" aria-hidden="true"></canvas>`;
  host.appendChild(overlay);
  return overlay;
}

function getSlotWinCeremonyConfig(type) {
  if (type === "legend") {
    return {
      tier: "legend",
      particles: 520,
      ...
      storm: 0.92,
      scale: [0.46, 1.08],
      mix: ["dust", "dust", "dust", "coin", "coin", "coin", "ruby", "blueGem", "greenGem", "scarab", "sun", "scroll", "ankh", "orb"],
    };
  }
  if (type === "mega" || type === "free-win") {
    return {
      tier: "mega",
      particles: 380,
      lightning: 0,
      ...
      storm: 0.76,
      scale: [0.44, 0.96],
      mix: ["dust", "dust", "coin", "coin", "coin", "ruby", "blueGem", "greenGem", "scarab", "sun"],
    };
  }
  if (type === "big") {
    return {
      ...
      storm: 0.62,
      scale: [0.42, 0.92],
      mix: ["dust", "dust", "coin", "coin", "coin", "ruby", "blueGem", "greenGem", "scarab"],
    };
  }
  return null;
}

function getSlotWinTreasureImage(kind) {
  const src = SLOT_WIN_TREASURE_PARTICLES[kind] || SLOT_WIN_TREASURE_PARTICLES.coin;
  if (slotWinTreasureImageCache.has(src)) return slotWinTreasureImageCache.get(src);
  const image = new Image();
  image.decoding = "async";
  image.src = src;
  slotWinTreasureImageCache.set(src, image);
  return image;
}

function paintSlotWinTreasureSprite(canvas, kind) {
  const spriteSize = SLOT_WIN_TREASURE_SPRITE_SIZE;
  ...
  if (kind === "dust") {
    ...
    const glow = spriteCtx.createRadialGradient(center, center, 0, center, center, 46);
    glow.addColorStop(0, "rgba(255, 249, 196, 0.98)");
    glow.addColorStop(0.28, "rgba(255, 207, 88, 0.64)");
    glow.addColorStop(0.62, "rgba(255, 177, 52, 0.2)");
    glow.addColorStop(1, "rgba(255, 172, 52, 0)");
    spriteCtx.fillStyle = glow;
    spriteCtx.fillRect(0, 0, spriteSize, spriteSize);
    return;
  }

  const image = getSlotWinTreasureImage(kind);
  const drawImage = () => {
    spriteCtx.clearRect(0, 0, spriteSize, spriteSize);
    spriteCtx.save();
    spriteCtx.shadowColor = kind === "ruby" ? "rgba(255, 97, 215, 0.76)" : "rgba(255, 224, 115, 0.7)";
    spriteCtx.shadowBlur = 16;
    spriteCtx.drawImage(image, 24, 24, 80, 80);
    spriteCtx.restore();
  };
  ...
  const fallback = spriteCtx.createRadialGradient(center, center, 0, center, center, 42);
  fallback.addColorStop(0, kind === "ruby" ? "#fff0ff" : "#fff4b8");
  fallback.addColorStop(0.48, kind === "ruby" ? "#ff55cd" : "#ffd24d");
  fallback.addColorStop(1, "rgba(255,255,255,0)");
  spriteCtx.fillStyle = fallback;
  spriteCtx.fillRect(0, 0, spriteSize, spriteSize);
  image.addEventListener("load", drawImage, { once: true });
}

function getSlotWinTreasureSprite(kind) {
  if (slotWinTreasureSpriteCache.has(kind)) return slotWinTreasureSpriteCache.get(kind);
  const canvas = document.createElement("canvas");
  canvas.width = SLOT_WIN_TREASURE_SPRITE_SIZE;
  canvas.height = SLOT_WIN_TREASURE_SPRITE_SIZE;
  paintSlotWinTreasureSprite(canvas, kind);
  slotWinTreasureSpriteCache.set(kind, canvas);
  return canvas;
}

function drawSlotWinTreasureParticle(ctx, x, y, size, rotation, alpha, kind = "coin", depth = 1) {
  const sprite = getSlotWinTreasureSprite(kind);
  ctx.save();
  ctx.translate(x, y);
  ctx.rotate(rotation);
  ctx.globalAlpha = kind === "dust" ? alpha * 0.82 : alpha;
  ctx.globalCompositeOperation = kind === "dust" ? "lighter" : "source-over";
  const wobble = kind === "coin" ? 0.86 + Math.abs(Math.sin(rotation)) * 0.16 : 1;
  ctx.scale(wobble, 1);
  const renderSize = kind === "dust" ? size * 1.62 : size * 1.6;
  ctx.drawImage(sprite, -renderSize / 2, -renderSize / 2, renderSize, renderSize);
  ctx.restore();
}

function drawSlotWinLightning(ctx, width, height, seed, progress, intensity = 1) {
  const side = seed % 2 === 0 ? -1 : 1;
  const startX = side < 0 ? width * (0.05 + (seed % 3) * 0.05) : width * (0.95 - (seed % 3) * 0.05);
  const startY = height * (0.1 + ((seed * 37) % 100) / 800);
  const endX = width * (0.34 + ((seed * 53) % 100) / 320);
  const endY = height * (0.34 + ((seed * 29) % 100) / 360);
  const segments = 7;
  ctx.save();
  ctx.globalCompositeOperation = "lighter";
  for (let pass = 0; pass < 2; pass += 1) {
    ...
    for (let i = 0; i <= segments; i += 1) {
      const t = i / segments;
      const wave = Math.sin((t * 9 + seed * 1.7 + progress * 4) * Math.PI) * width * 0.025 * intensity;
      const x = startX + (endX - startX) * t + wave * (i % 2 ? 1 : -1);
      const y = startY + (endY - startY) * t + Math.cos(seed + t * 7) * height * 0.012;
      if (i === 0) ctx.moveTo(x, y);
      else ctx.lineTo(x, y);
    }
    ctx.strokeStyle = pass === 0 ? `rgba(255, 210, 96, ${0.16 * intensity})` : `rgba(255, 246, 182, ${0.5 * intensity})`;
    ctx.lineWidth = pass === 0 ? 9 * intensity : 1.6 * intensity;
    ctx.shadowColor = "rgba(255, 93, 221, 0.72)";
    ctx.shadowBlur = pass === 0 ? 18 : 8;
    ctx.stroke();
  }
  ctx.restore();
...
function stopSlotWinCeremony() {
  const slot = state.slot;
  if (slot.winCeremonyFrame) {
    window.cancelAnimationFrame(slot.winCeremonyFrame);
    slot.winCeremonyFrame = null;
  }
  const overlay = $("#slot-win-ceremony");
  if (overlay) {
...
function startSlotWinCeremony(type, amount, preferredDuration = 3000) {
  const config = getSlotWinCeremonyConfig(type);
  if (!config) {
    stopSlotWinCeremony();
    return;
  }
  const slot = state.slot;
  const overlay = ensureSlotWinCeremonyOverlay();
  const canvas = overlay?.querySelector("canvas");
  if (!overlay || !canvas) return;
  if (slot.winCeremonyFrame) window.cancelAnimationFrame(slot.winCeremonyFrame);

  const duration = Math.max(config.duration, preferredDuration || 0);
  const rect = overlay.getBoundingClientRect();
  const dpr = Math.min(1.18, window.devicePixelRatio || 1);
  ...
  const controlsRect = $("#screen-pvp .pharaoh-controls")?.getBoundingClientRect();
  const gridRect = $("#slot-grid")?.getBoundingClientRect();
  const presentationTop = Number.parseFloat(state.slot.presentation?.fxTop || state.slot.presentation?.presentationTop || 0);
  const protectedTop = gridRect ? Math.max(0, gridRect.top - rect.top + 4) : height * 0.22;
  const protectedBottom = controlsRect
    ? Math.max(height * 0.42, controlsRect.top - rect.top - 26)
    : height * 0.78;
  const safeFallBottom = controlsRect
    ? Math.max(height * 0.5, Math.min(height * 0.78, controlsRect.top - rect.top - 30))
    : height * 0.86;
  const focusY = presentationTop || (gridRect ? gridRect.top - rect.top + gridRect.height * 0.5 : height * 0.36);
  [...new Set(config.mix)].forEach((kind) => getSlotWinTreasureSprite(kind));
  const items = [];
  const total = config.particles;
  for (let index = 0; index < total; index += 1) {
    const spread = index / Math.max(1, total - 1);
    const depth = 0.5 + Math.random() * 0.96;
    const kind = config.mix[Math.floor(Math.random() * config.mix.length)] || "coin";
    const isDust = kind === "dust";
    const sizeBase = isDust ? 5 + Math.random() * 9 : 16 + Math.random() * 24;
    const sideBias = index % 6 === 0 ? (Math.random() < 0.5 ? width * 0.06 : width * 0.94) : width * (0.03 + Math.random() * 0.94);
    items.push({
      kind,
      depth,
      x: sideBias,
      y: protectedTop - height * (0.02 + Math.random() * 0.34),
      vx: (Math.random() - 0.5) * (10 + config.storm * 22),
      vy: height * (0.105 + Math.random() * 0.17 + config.storm * 0.045) * depth,
      size: sizeBase * (config.scale[0] + Math.random() * (config.scale[1] - config.scale[0])),
      spin: (Math.random() - 0.5) * 6,
      rotation: Math.random() * Math.PI * 2,
      delay: spread * duration * 0.42 + Math.random() * 120,
      drift: Math.random() * Math.PI * 2,
      sway: isDust ? 7 + Math.random() * 18 : 9 + Math.random() * 20,
    });
  }

  overlay.dataset.tier = config.tier;
  overlay.classList.add("show");
  overlay.setAttribute("aria-hidden", "false");
  const startedAt = performance.now();
  const coarsePointer = window.matchMedia?.("(pointer: coarse)")?.matches;
  const constrainedCpu = (navigator.hardwareConcurrency || 8) <= 4;
  const targetFrameInterval = coarsePointer || constrainedCpu ? 1000 / 45 : 1000 / 60;
  let lastRenderedAt = 0;

  const draw = (now) => {
    const elapsed = now - startedAt;
    const progress = Math.min(1, elapsed / duration);
    if (progress < 1 && lastRenderedAt && now - lastRenderedAt < targetFrameInterval) {
      slot.winCeremonyFrame = window.requestAnimationFrame(draw);
      return;
    }
    lastRenderedAt = now;
    ...
    for (let itemIndex = 0; itemIndex < items.length; itemIndex += 1) {
      ...
      const y = item.y + item.vy * (local / 1000) * 2.75;
      const x = item.x + Math.sin(item.drift + local / 330) * item.sway + item.vx * (local / 1000);
      if (y > safeFallBottom + item.size + 20) {
        if (progress < 0.9) {
          item.kind = config.mix[Math.floor(Math.random() * config.mix.length)] || "coin";
          item.depth = 0.5 + Math.random() * 0.96;
          item.y = protectedTop - item.size - Math.random() * height * 0.2;
          item.x = width * (0.03 + Math.random() * 0.94);
          item.delay = elapsed + Math.random() * 70;
          item.vx = (Math.random() - 0.5) * (10 + config.storm * 22);
          item.vy = height * (0.105 + Math.random() * 0.17 + config.storm * 0.045) * item.depth;
        }
        continue;
      }
      if (y < protectedTop) continue;
      const itemAlpha = Math.min(1, t / 0.08) * alpha;
      const presentationFade = Math.abs(y - focusY) < 76 && Math.abs(x - width * 0.5) < Math.min(width * 0.4, 185) ? 0.04 : 1;
      const bottomFade = y > safeFallBottom - 64 ? Math.max(0, 1 - (y - (safeFallBottom - 64)) / 64) : 1;
      const topFade = y < protectedTop + 54 ? Math.max(0, (y - protectedTop) / 54) : 1;
      const frontAlpha = itemAlpha * bottomFade * topFade * presentationFade * Math.min(1, 0.78 + item.depth * 0.22);
      drawSlotWinTreasureParticle(ctx, x, y, item.size, item.rotation + local * item.spin * 0.0012, frontAlpha, item.kind, item.depth);
    }

    if (config.lightning > 0 && progress > 0.1 && progress < 0.84) {
      const pulse = (Math.sin(elapsed / 155) + 1) / 2;
      for (let index = 0; index < config.lightning; index += 1) {
        if (((elapsed / 180 + index * 1.7) % 4) < 1.45) {
          drawSlotWinLightning(ctx, width, height, index + 11, progress, (0.55 + pulse * 0.45) * config.storm);
        }
      }
    }
...
function getSlotPresentationDuration(presentation = {}, requestedDuration = 1550) {
  const type = presentation.type || "idle";
  if (type === "legend") return Math.max(requestedDuration, 11200);
  if (type === "mega" || type === "free-win") return Math.max(requestedDuration, 9000);
  if (type === "big") return Math.max(requestedDuration, 7400);
  if ((presentation.win || 0) > 0) return Math.max(requestedDuration, 1800);
  return requestedDuration;
}

function getSlotPresentationCountDuration(presentation = {}, totalDuration = 1550) {
  const type = presentation.type || "idle";
  if (type === "legend") return Math.min(totalDuration - 1250, 9800);
  if (type === "mega" || type === "free-win") return Math.min(totalDuration - 980, 7800);
  if (type === "big") return Math.min(totalDuration - 760, 6400);
  return Math.min(totalDuration - 420, Math.max(780, totalDuration * 0.72));
}

function getSlotRoundPresentationDuration(presentation, requestedDuration = 3200) {
  if (!presentation) return 0;
  return getSlotPresentationDuration(presentation, requestedDuration);
}

function getSlotFreeSpinsLabel(slot = state.slot) {
  const count = Math.max(0, slot.freeSpins || 0);
  if (count > 0) return `FREE SPINS ${count}`;
  return "FREE SPINS 0";
}

function activateSlotPrizeFrame(type, duration = 2600) {
  const slot = state.slot;
  const gridRoot = $("#slot-grid");
  const strongTiers = new Set(["big", "mega", "legend", "free-win"]);
  if (!gridRoot || !strongTiers.has(type)) return;
  if (slot.prizeFrameTimer) {
    window.clearTimeout(slot.prizeFrameTimer);
    slot.prizeFrameTimer = null;
  }
  const tier = type === "free-win" ? "mega" : type;
  gridRoot.dataset.prizeTier = tier;
  gridRoot.classList.remove("slot-grid-prize-frame");
  void gridRoot.offsetWidth;
  gridRoot.classList.add("slot-grid-prize-frame");
  const hold = tier === "legend" ? Math.max(duration, 3600)
    : tier === "mega" ? Math.max(duration, 3100)
    : Math.max(duration, 2400);
  slot.prizeFrameTimer = window.setTimeout(() => {
    if (gridRoot.dataset.prizeTier === tier) {
      delete gridRoot.dataset.prizeTier;
      gridRoot.classList.remove("slot-grid-prize-frame");
    }
    slot.prizeFrameTimer = null;
...
function animateSlotPresentationValue(presentation, duration = 900) {
  const slot = state.slot;
  const target = Math.max(0, presentation.win || 0);
  if (!target) return;
  const startedAt = performance.now();
  const countDuration = Math.max(680, getSlotPresentationCountDuration(presentation, duration));
  presentation.counting = true;
  presentation.settled = false;
  const tick = (now) => {
    if (state.slot.presentation !== presentation) return;
    const progress = Math.min(1, (now - startedAt) / countDuration);
    const eased = progress < 0.82
      ? progress * 0.9
      : 0.738 + (1 - Math.pow(1 - (progress - 0.82) / 0.18, 2.35)) * 0.262;
    presentation.displayWin = Math.max(1, Math.round(target * eased));
    window.mirageAudio?.pharaohPayoutTick(progress, presentation.type);
    renderSlotPresentation(slot);
    if (progress < 1) {
      slot.presentationFrame = window.requestAnimationFrame(tick);
      return;
    }
    presentation.displayWin = target;
    presentation.counting = false;
    presentation.settled = true;
    slot.presentationFrame = null;
    window.mirageAudio?.pharaohPayoutComplete(presentation.type);
    renderSlotPresentation(slot);
  };
  slot.presentationFrame = window.requestAnimationFrame(tick);
}
...
function showSlotPresentation(presentation, duration = 1550) {
  ...
  slot.presentation = { win: 0, displayWin: 0, ruby: false, ...presentation };
  syncNavigationLock();
  const activePresentation = slot.presentation;
  const presentationDuration = getSlotPresentationDuration(activePresentation, duration);
  if (activePresentation.type !== "free" && activePresentation.win > 0) {
    window.mirageAudio?.pharaohPayoutStart(activePresentation.type);
  }
  activePresentation.durationMs = presentationDuration;
  const gridRoot = $("#slot-grid");
  if (gridRoot) {
    const gridRect = gridRoot.getBoundingClientRect();
    activePresentation.fxTop = gridRect.top + gridRect.height * 0.5;
  }
  if (activePresentation.win > 0) {
    activePresentation.displayWin = Math.max(1, Math.round(activePresentation.win * 0.015));
  }
  const winbar = $("#slot-winbar");
  if (winbar && slot.presentation.type !== "free") {
    const strong = ["big", "mega", "legend", "free-win"].includes(slot.presentation.type);
    winbar.classList.remove("pulse", "big-pulse");
    void winbar.offsetWidth;
    winbar.classList.add(strong ? "big-pulse" : "pulse");
  }
  activateSlotPrizeFrame(slot.presentation.type, presentationDuration);
  if (SLOT_WIN_CEREMONY_TIERS.has(slot.presentation.type)) {
    startSlotWinCeremony(slot.presentation.type, slot.presentation.win, presentationDuration);
  }
  animateSlotPresentationValue(activePresentation, presentationDuration);
  slot.presentationTimer = window.setTimeout(() => {
    if (state.slot.presentation === activePresentation) {
      if (slot.presentationFrame) window.cancelAnimationFrame(slot.presentationFrame);
      state.slot.presentation = { type: "idle", title: "", value: "", note: "", win: 0, displayWin: 0, ruby: false };
      state.slot.presentationFrame = null;
...
async function showSlotPresentationAfterSettle(presentation, { duration = 1550, delay = 260 } = {}) {
  if (!presentation) return;
  if (delay > 0) await sleep(delay);
  showSlotPresentation(presentation, duration);
  renderSlotPresentation(state.slot);
}
...
function getSlotWinPresentation(result, { bonusBuy = false, isFreeSpin = false } = {}) {
  ...
  if (result.freeSpinsAwarded > 0) {
    return {
      type: "free",
      title: "ДАР ДЖИНА",
      value: `${result.freeSpinsAwarded} FREE SPINS`,
      note: result.totalWin > 0 ? `Итог +${format(result.totalWin)} 💎` : "Серия запущена",
    };
  }
  if (result.freeSpinsPlayed > 0 || isFreeSpin) {
    ...
    return {
      ...
      win: result.totalWin,
      ruby: true,
      note: formatSlotSpinCount(result.freeSpinsPlayed || 1),
    };
  }
  if (result.totalWin <= 0) return null;
  const multiplier = result.totalWin / Math.max(1, state.slot.stake);
  const explanation = buildSlotResultExplanation(result);
  const note = explanation?.summary || `${floorMultiplier(multiplier).toFixed(2)}x`;
  if (multiplier >= 100) {
    return { type: "legend", title: "ВОЛЯ ФАРАОНА", value: "", win: result.totalWin, ruby: true, note };
  }
  if (multiplier >= 30) {
    return { type: "mega", title: "СОКРОВИЩЕ", value: "", win: result.totalWin, ruby: true, note };
  }
  if (multiplier >= 10) {
    return { type: "big", title: "ДАР ЛАМПЫ", value: "", win: result.totalWin, ruby: true, note };
  }
  return { type: "win", title: "ВЫИГРЫШ", value: "", win: result.totalWin, ruby: true, note };
}
...
function updateSlotGridCell(cell, cellValue, rowIndex, colIndex, winningKeys) {
  const symbolId = getSlotCellSymbolId(cellValue);
  const multiplierValue = getSlotCellMultiplierValue(cellValue);
  const symbol = getSlotSymbol(symbolId);
  ...
  cell.setAttribute("aria-label", multiplierValue > 0 ? `${symbol.name} x${multiplierValue}` : symbol.name);
  if (cell.dataset.cellValue === cellValue) return;
  cell.dataset.symbol = symbolId;
  cell.dataset.cellValue = cellValue;
  cell.innerHTML = "";
  cell.appendChild(createSlotSymbolIcon(symbol, "slot-symbol-orb", multiplierValue));
}
...
function updateSlotGenieReaction(slot) {
  ...
  const debugState = new URLSearchParams(window.location.search).get("debugGenieState");
  if (SLOT_GENIE_DEBUG_STATES.has(debugState)) {
    reaction.state = debugState;
    reaction.text = SLOT_GENIE_DEBUG_TEXT[debugState] || reaction.text;
  }
  const previousReaction = genie.dataset.reaction || "idle";
  if (previousReaction !== reaction.state) {
    genie.classList.remove("genie-state-change");
    void genie.offsetWidth;
    genie.classList.add("genie-state-change");
    window.clearTimeout(genie._reactionTimer);
    genie._reactionTimer = window.setTimeout(() => {
      genie.classList.remove("genie-state-change");
    }, 520);
  }
  genie.dataset.reaction = reaction.state;
  const genieImage = genie.querySelector(".slot-genie-lamp img");
  const asset = SLOT_GENIE_REACTION_ASSETS[reaction.state] || SLOT_GENIE_REACTION_ASSETS.idle;
  if (genieImage && genieImage.getAttribute("src") !== asset) {
    genieImage.setAttribute("src", asset);
  }
...
function showSlotScatterAward() {
  const gridRoot = $("#slot-grid");
  const genieRoot = $("#slot-genie-reaction");
  if (!gridRoot || !genieRoot) return;
  window.mirageAudio?.pharaohScatter();
  gridRoot.classList.remove("slot-grid-scatter-award");
  genieRoot.classList.remove("slot-genie-scatter-award");
  void gridRoot.offsetWidth;
  gridRoot.classList.add("slot-grid-scatter-award");
  genieRoot.classList.add("slot-genie-scatter-award");
  window.setTimeout(() => {
    gridRoot.classList.remove("slot-grid-scatter-award");
    genieRoot.classList.remove("slot-genie-scatter-award");
  }, 3100);
...
function applySlotPrizeFrameDebug(gridRoot = $("#slot-grid")) {
  if (!gridRoot) return;
  const tier = new URLSearchParams(window.location.search).get("debugPrizeFrame");
  if (!["big", "mega", "legend"].includes(tier)) return;
  gridRoot.dataset.prizeTier = tier;
  gridRoot.classList.add("slot-grid-prize-frame");
}

function renderSlotHistory(slot = state.slot) {
  const history = $("#slot-history");
  if (!history) return;
  const explanation = slot.explanation;
  history.innerHTML = "";
  history.classList.toggle("explain", Boolean(explanation?.items?.length));

  if (explanation?.items?.length) {
    explanation.items.slice(0, 5).forEach((item, index) => {
      const chip = document.createElement("span");
      chip.className = `slot-history-chip explain explain-${index}`;
...
function renderPharaoh() {
  ...
  if (gridRoot) {
    ...
    applySlotPrizeFrameDebug(gridRoot);
  ...
  const status = $("#slot-status");
  if (status) updateSlotStatus(slot);
  renderSlotMultiplierMeter(slot);
  renderSlotPresentation(slot);
  ...
  if (freeSpins) {
    const freeText = getSlotFreeSpinsLabel(slot);
    const autoText = slot.autoRunning ? `АВТО ${slot.autoRunsRemaining}` : "";
    freeSpins.textContent = autoText ? `${freeText} · ${autoText}` : freeText;
    freeSpins.classList.toggle("active", slot.freeSpins > 0 || slot.autoRunning || slot.bonusSessionActive);
  }
  renderSlotHistory(slot);
  updateSlotGenieReaction(slot);
  const spinButton = $("#slot-spin");
  if (spinButton) {
    const canStop = slot.autoRunning || slot.bonusSessionRunning;
    spinButton.disabled = slot.status !== "READY" && !canStop;
    spinButton.querySelector("strong").textContent = canStop ? "СТОП" : "СПИН";
  }
  ...
  const bonusBuyLimit = getSlotBonusBuyLimit();
  const bonusWithinPool = bonusCost <= bonusBuyLimit;
  const bonusAvailable = SLOT_CONFIG.bonusBuy.enabled && slot.status === "READY" && !slot.autoRunning && !slot.bonusSessionActive && slot.freeSpins === 0 && bonusCost <= state.balance && bonusWithinPool;
  if (bonus) {
    bonus.disabled = !bonusAvailable;
    bonus.classList.toggle("available", bonusAvailable);
    bonus.textContent = SLOT_CONFIG.bonusBuy.enabled ? "КУПИТЬ 15 FS" : "БОНУС СКОРО";
    bonus.title = !SLOT_CONFIG.bonusBuy.enabled ? "Бонус будет включен позже"
      : !bonusWithinPool ? `Лимит покупки: до ${format(bonusBuyLimit)} рубинов при пуле ${format(SLOT_CONFIG.roomPoolUsd)}`
      : `Купить 15 free spins за ${format(bonusCost)} рубинов`;
  }
  ...
  if (bonusNote) {
    bonusNote.innerHTML = bonusWithinPool
      ? `${SLOT_CONFIG.bonusBuy.freeSpins} free spins по цене ${SLOT_CONFIG.bonusBuy.costMultiplier} обычных спинов. Ставка ${format(slot.stake)} <i class="stake-ruby" aria-hidden="true"></i>, цена серии ${format(bonusCost)} <i class="stake-ruby" aria-hidden="true"></i>. 5 спинов в подарок.`
      : `Покупка недоступна: цена серии ${format(bonusCost)} <i class="stake-ruby" aria-hidden="true"></i> выше лимита пула ${format(bonusBuyLimit)} <i class="stake-ruby" aria-hidden="true"></i>. Уменьши ставку.`;
  }
...
function getCarpetMotionResponse(elapsed, tau) {
  const ratio = Math.max(0, elapsed) / tau;
  return 1 - (1 + ratio) * Math.exp(-ratio);
}

function getCarpetMotionDerivative(elapsed, tau) {
  const time = Math.max(0, elapsed);
  return (time * Math.exp(-time / tau)) / (tau * tau);
}

function getCarpetPathProgress(elapsed) {
  const fast = getCarpetMotionResponse(elapsed, CARPET_FAST_MOTION_TAU_MS);
  const slow = getCarpetMotionResponse(elapsed, CARPET_SLOW_MOTION_TAU_MS);
  return Math.min(0.999, CARPET_FAST_PATH_SHARE * fast + (1 - CARPET_FAST_PATH_SHARE) * slow);
}

function getCarpetPathGeometry() {
  const element = document.querySelector("#carpet-motion-geometry");
  if (!element || typeof element.getTotalLength !== "function") return null;
  if (carpetMotionGeometry.element === element && carpetMotionGeometry.length > 0) return carpetMotionGeometry;

  const length = element.getTotalLength();
  if (!Number.isFinite(length) || length <= 0) return null;
  const delta = Math.max(1, length * 0.006);
  const startPoint = element.getPointAtLength(0);
  const startNext = element.getPointAtLength(delta);
  const endPrevious = element.getPointAtLength(Math.max(0, length - delta));
  const endPoint = element.getPointAtLength(length);

  carpetMotionGeometry.element = element;
  carpetMotionGeometry.length = length;
  carpetMotionGeometry.startPoint = startPoint;
  carpetMotionGeometry.startHeading = Math.atan2(startNext.y - startPoint.y, startNext.x - startPoint.x);
  carpetMotionGeometry.endHeading = Math.atan2(endPoint.y - endPrevious.y, endPoint.x - endPrevious.x);
  return carpetMotionGeometry;
}

function getCarpetPointOnPath(progress) {
  const geometry = getCarpetPathGeometry();
  if (!geometry) {
    return {
      x: CARPET_PATH_START_X + (CARPET_PATH_END_X - CARPET_PATH_START_X) * progress,
      y: CARPET_PATH_START_Y + (CARPET_PATH_END_Y - CARPET_PATH_START_Y) * progress,
      rotate: -8 + progress * 12,
    };
  }

  const distance = geometry.length * progress;
  const tangentDelta = Math.max(1, geometry.length * 0.006);
  const point = geometry.element.getPointAtLength(distance);
  const before = geometry.element.getPointAtLength(Math.max(0, distance - tangentDelta));
  const after = geometry.element.getPointAtLength(Math.min(geometry.length, distance + tangentDelta));
  const heading = Math.atan2(after.y - before.y, after.x - before.x);
  const headingRange = geometry.endHeading - geometry.startHeading;
  const headingProgress = Math.max(0, Math.min(1, headingRange ? (heading - geometry.startHeading) / headingRange : progress));

  return {
    x: point.x,
    y: point.y,
    rotate: -8 + headingProgress * 12,
  };
}

function getCarpetHeroGeometry() {
  const element = document.querySelector("#carpet-hero-wrap");
  if (!element) return carpetHeroGeometry;
  if (carpetHeroGeometry.element === element) return carpetHeroGeometry;

  const width = element.offsetWidth || CARPET_HERO_WIDTH;
  const height = element.offsetHeight || CARPET_HERO_HEIGHT;
  const origin = getComputedStyle(element).transformOrigin.split(" ").map((value) => Number.parseFloat(value));
  carpetHeroGeometry.element = element;
  carpetHeroGeometry.baseX = element.offsetLeft;
  carpetHeroGeometry.baseY = element.offsetTop;
  carpetHeroGeometry.originX = Number.isFinite(origin[0]) ? origin[0] : width * 0.56;
  carpetHeroGeometry.originY = Number.isFinite(origin[1]) ? origin[1] : height * 0.62;
  carpetHeroGeometry.engineX = width * 0.18;
  carpetHeroGeometry.engineY = height * 0.73;
  return carpetHeroGeometry;
}

function getCarpetTranslationForEngine(point, scale, rotate) {
  const geometry = getCarpetHeroGeometry();
  const radians = rotate * Math.PI / 180;
  const localX = (geometry.engineX - geometry.originX) * scale;
  const localY = (geometry.engineY - geometry.originY) * scale;
  ...
  return {
    x: point.x - geometry.baseX - geometry.originX - rotatedX,
    y: point.y - geometry.baseY - geometry.originY - rotatedY,
  };
}

function getCarpetVisualPose(carpet, now = performance.now()) {
  if (carpet.status === "READY" || !carpet.roundStartedAt) {
    return { x: 0, y: 0, scale: 1, rotate: -8, speed: 0, pathProgress: 0, accelerating: false };
  }

  if (carpet.status !== "FLYING") {
    return {
      x: carpet.visualX,
      y: carpet.visualY,
      scale: carpet.visualScale,
      rotate: carpet.visualRotate,
      pathProgress: carpet.visualProgress,
      speed: 1,
      accelerating: false,
    };
  }

  const elapsed = Math.max(0, now - carpet.roundStartedAt);
  const pathProgress = getCarpetPathProgress(elapsed);
  const point = getCarpetPointOnPath(pathProgress);
  const scale = 1 - pathProgress * 0.02;
  const translation = getCarpetTranslationForEngine(point, scale, point.rotate);
  const derivative =
    CARPET_FAST_PATH_SHARE * getCarpetMotionDerivative(elapsed, CARPET_FAST_MOTION_TAU_MS)
    + (1 - CARPET_FAST_PATH_SHARE) * getCarpetMotionDerivative(elapsed, CARPET_SLOW_MOTION_TAU_MS);
  const speed = Math.max(0.2, Math.min(1, derivative / 0.00094));
  ...
  return {
    ...
    speed,
    pathProgress,
    accelerating: elapsed < CARPET_ACCELERATION_MS,
  };
}
...
function carpetDissolveNoise(x, y) {
  const value = Math.sin(x * 127.1 + y * 311.7) * 43758.5453123;
  return value - Math.floor(value);
}

function buildCarpetDissolveParticles() {
  carpetDissolve.particles = Array.from({ length: 52 }, (_, index) => {
    const angle = carpetDissolveNoise(index, 4) * Math.PI * 2;
    const radius = 18 + carpetDissolveNoise(index, 9) * 42;
    return {
      x: Math.cos(angle) * radius,
      y: Math.sin(angle) * radius * 0.72,
      controlX: 14 + carpetDissolveNoise(index, 13) * 30,
      controlY: (carpetDissolveNoise(index, 17) - 0.5) * 50,
      size: 0.75 + carpetDissolveNoise(index, 21) * 1.9,
      delay: carpetDissolveNoise(index, 25) * 0.28,
      hue: index % 4,
    };
  });
}

function prepareCarpetDissolveFrames() {
  if (carpetDissolve.ready || carpetDissolve.preparing) return;
  const hero = document.querySelector("#carpet-hero-wrap .carpet-hero");
  if (!hero) return;
  if (!hero.complete || !hero.naturalWidth) {
    carpetDissolve.preparing = true;
    hero.addEventListener("load", () => {
      carpetDissolve.preparing = false;
      prepareCarpetDissolveFrames();
    }, { once: true });
    return;
  }
  ...
  try {
    ...
    sourceContext.drawImage(hero, 0, 0, width, height);
    const base = sourceContext.getImageData(0, 0, width, height);
    carpetDissolve.frames = [];

    for (let frameIndex = 0; frameIndex < CARPET_DISSOLVE_FRAME_COUNT; frameIndex += 1) {
      const frame = document.createElement("canvas");
      const frameContext = frame.getContext("2d");
      const output = new ImageData(new Uint8ClampedArray(base.data), width, height);
      const phase = frameIndex / (CARPET_DISSOLVE_FRAME_COUNT - 1);
      frame.width = width;
      frame.height = height;
      ...
      for (let y = 0; y < height; y += 1) {
        for (let x = 0; x < width; x += 1) {
          ...
          const alpha = base.data[offset + 3];
          if (alpha === 0) continue;
          const nx = x / (width - 1);
          const ny = y / (height - 1);
          const directional = nx * 0.68 + (1 - ny) * 0.32;
          const fine = carpetDissolveNoise(x, y);
          const broad = carpetDissolveNoise(Math.floor(x / 7), Math.floor(y / 7));
          const score = directional * 0.65 + fine * 0.22 + broad * 0.13;
          const threshold = -0.06 + phase * 1.12;
          const edgeDistance = score - threshold;

          if (edgeDistance < 0) {
            output.data[offset + 3] = 0;
          } else if (edgeDistance < 0.055) {
            const strength = 1 - edgeDistance / 0.055;
            output.data[offset] = Math.min(255, output.data[offset] + 95 * strength);
            output.data[offset + 1] = Math.min(255, output.data[offset + 1] + 34 * strength);
            output.data[offset + 2] = Math.min(255, output.data[offset + 2] + 112 * strength);
            output.data[offset + 3] = Math.min(255, alpha + 30 * strength);
          }
        }
      }

      frameContext.putImageData(output, 0, 0);
      carpetDissolve.frames.push(frame);
    }

    buildCarpetDissolveParticles();
    carpetDissolve.ready = true;
  } catch (error) {
    console.warn("Carpet dissolve frames could not be prepared", error);
  } finally {
    carpetDissolve.preparing = false;
  }
}

function renderCarpetDissolve(progress = 0) {
  const canvas = $("#carpet-dissolve-canvas");
  if (!canvas) return;
  prepareCarpetDissolveFrames();
  ...
  if (!carpetDissolve.ready || state.carpet.status !== "CRASHING") return;

  const phase = clampUnit(progress);
  const collapse = easeInOutCubic(phase);
  const frameIndex = Math.min(
    CARPET_DISSOLVE_FRAME_COUNT - 1,
    Math.floor(phase * CARPET_DISSOLVE_FRAME_COUNT),
  );
  const origin = { x: 99, y: 108.5 };
  const target = { x: 171, y: 54.5 };
  const pullX = origin.x + (target.x - origin.x) * collapse * 0.38;
  const pullY = origin.y + (target.y - origin.y) * collapse * 0.38;
  const heroWidth = 102 * (1 - collapse * 0.12);
  const heroHeight = 153 * (1 - collapse * 0.12);

  context.save();
  context.translate(pullX, pullY);
  context.rotate(collapse * 0.08);
  context.drawImage(
    carpetDissolve.frames[frameIndex],
    -heroWidth * 0.5,
    -heroHeight * 0.5,
    heroWidth,
    heroHeight,
  ...
  const colors = ["255,224,145", "219,108,255", "255,176,232", "174,112,255"];
  carpetDissolve.particles.forEach((particle) => {
    const local = clampUnit((phase - particle.delay) / (1 - particle.delay));
    if (local <= 0 || local >= 1) return;
    const inverse = 1 - local;
    const sourceX = origin.x + particle.x;
    const sourceY = origin.y + particle.y;
    const controlX = origin.x + particle.controlX;
    const controlY = origin.y + particle.controlY;
    const x = inverse * inverse * sourceX + 2 * inverse * local * controlX + local * local * target.x;
    const y = inverse * inverse * sourceY + 2 * inverse * local * controlY + local * local * target.y;
    const alpha = Math.sin(local * Math.PI) * 0.84;
    context.fillStyle = `rgba(${colors[particle.hue]},${alpha})`;
    context.beginPath();
    context.arc(x, y, particle.size * (1 - local * 0.42), 0, Math.PI * 2);
    context.fill();
  });

  context.translate(target.x, target.y);
  context.rotate(phase * Math.PI * 1.7);
  for (let ring = 0; ring < 3; ring += 1) {
    const radius = (27 - ring * 6) * (1 - phase * 0.72);
    context.beginPath();
    context.ellipse(0, 0, radius, radius * 0.34, ring * 0.7, 0.2, Math.PI * 1.72);
    context.strokeStyle = ring === 1
      ? `rgba(255,225,143,${0.72 * (1 - phase)})`
      : `rgba(208,91,255,${0.58 * (1 - phase)})`;
    context.lineWidth = ring === 1 ? 1.6 : 1.15;
    context.stroke();
  }
  context.restore();
}
...
function updateBalanceDisplays() {
  const headerBalanceIds = new Set(["balance-value", "solo-balance", "carpet-balance", "slot-balance", "pvp-balance"]);
  ["balance-value", "solo-balance", "carpet-balance", "slot-balance", "pvp-balance", "account-header-balance", "account-balance", "account-wallet-balance"].forEach((id) => {
    const el = document.getElementById(id);
    if (!el) return;
    el.textContent = headerBalanceIds.has(id) ? formatHeaderBalance(state.balance) : format(state.balance);
    const roomBalance = el.closest(".room-balance");
    if (roomBalance) {
      roomBalance.title = `${format(state.balance)} рубинов`;
      roomBalance.setAttribute("aria-label", `Баланс ${format(state.balance)} рубинов. Открыть пополнение`);
    }
  });
}
...
function renderRoomRules() {
  $$('[data-room-rules-panel]').forEach((panel) => {
    const open = panel.dataset.roomRulesPanel === state.rulesRoom;
    panel.classList.toggle('open', open);
    panel.setAttribute('aria-hidden', open ? 'false' : 'true');
  });
  $$('[data-room-rules-backdrop]').forEach((backdrop) => {
    backdrop.classList.toggle('open', backdrop.dataset.roomRulesBackdrop === state.rulesRoom);
  });
}

function setRoomRulesOpen(room = null) {
  state.rulesRoom = room;
  document.body.dataset.roomRules = room || "";
...
function renderCarpet() {
  ...
  const pose = getCarpetVisualPose(carpet);
  const { x, y, scale, rotate, speed, pathProgress, accelerating } = pose;
  carpet.visualX = x;
  carpet.visualY = y;
  carpet.visualScale = scale;
  carpet.visualRotate = rotate;
  carpet.visualProgress = pathProgress;

  stage.style.setProperty("--carpet-progress", progress.toFixed(4));
  stage.style.setProperty("--carpet-speed", speed.toFixed(4));
  stage.style.setProperty("--carpet-path-progress", pathProgress.toFixed(4));
  const crashCollapse = easeInOutCubic(carpet.crashAnimationProgress || 0);
  stage.style.setProperty("--carpet-crash-trail-length", (pathProgress * (1 - crashCollapse)).toFixed(4));
  stage.style.setProperty("--carpet-crash-trail-offset", (-pathProgress * crashCollapse).toFixed(4));
  const cashoutProgress = clampUnit(carpet.cashoutAnimationProgress || 0);
  const cashoutConfirm = clampUnit(carpet.cashoutConfirmProgress || 0);
  const cashoutCollapse = easeInOutCubic(clampUnit(cashoutProgress * 1.12));
  const cashoutTrailStart = pathProgress * cashoutCollapse;
  const cashoutTrailLength = Math.max(0, pathProgress - cashoutTrailStart);
  const cashoutFade = cashoutProgress < 0.68
    ? 1
    : 1 - (1 - Math.pow(1 - clampUnit((cashoutProgress - 0.68) / 0.32), 3));
  const cashoutFlare = Math.max(
    Math.sin(cashoutConfirm * Math.PI) * 0.96,
    Math.sin(cashoutProgress * Math.PI) * 0.82,
  );
  stage.style.setProperty("--carpet-cashout-progress", cashoutProgress.toFixed(4));
  stage.style.setProperty("--carpet-cashout-confirm", cashoutConfirm.toFixed(4));
  stage.style.setProperty("--carpet-cashout-hero-opacity", clampUnit(cashoutFade).toFixed(4));
  stage.style.setProperty("--carpet-cashout-flare-opacity", clampUnit(cashoutFlare).toFixed(4));
  stage.style.setProperty("--carpet-cashout-trail-length", cashoutTrailLength.toFixed(4));
  stage.style.setProperty("--carpet-cashout-trail-offset", (-cashoutTrailStart).toFixed(4));
  stage.classList.toggle("ready", carpet.status === "READY");
  stage.classList.toggle("accelerating", accelerating);
  stage.classList.toggle("flying", carpet.status === "FLYING");
  stage.classList.toggle("cashed", carpet.status === "CASHED_OUT");
  stage.classList.toggle("cashing-out", carpet.status === "CASHING_OUT");
  stage.classList.toggle("dissolving", carpet.status === "CRASHING");
  stage.classList.toggle("dissolve-ready", carpetDissolve.ready);
  stage.classList.toggle("message", ["CASHING_OUT", "CASHED_OUT", "CRASHED"].includes(carpet.status));
  stage.classList.toggle("crashed", carpet.status === "CRASHED");
  const screen = $("#screen-solo");
  if (screen) {
    screen.style.setProperty("--carpet-speed", speed.toFixed(4));
    screen.classList.toggle("carpet-ready", carpet.status === "READY");
    screen.classList.toggle("carpet-accelerating", accelerating);
    screen.classList.toggle("carpet-flying", carpet.status === "FLYING");
    screen.classList.toggle("carpet-cashed", carpet.status === "CASHED_OUT");
    screen.classList.toggle("carpet-cashing-out", carpet.status === "CASHING_OUT");
    screen.classList.toggle("carpet-dissolving", carpet.status === "CRASHING");
  ...
  renderCarpetDissolve(carpet.crashAnimationProgress || 0);
  ...
  if (status) {
    if (carpet.status === "FLYING") status.textContent = accelerating ? "НАБОР СКОРОСТИ" : "ПОЛЕТ";
    else if (carpet.status === "CASHING_OUT") status.textContent = "ВЫИГРЫШ ЗАФИКСИРОВАН";
    else if (carpet.status === "CASHED_OUT") status.textContent = `ЗАБРАНО ${format(carpet.payout)} КРИСТАЛЛОВ`;
    else if (["CRASHING", "CRASHED"].includes(carpet.status)) status.textContent = "БУРЯ НАСТИГЛА КОВЕР";
  ...
  if (label) {
    if (carpet.autoRunning && carpet.status === "READY") label.textContent = "СТОП";
    else if (carpet.status === "FLYING") label.textContent = "ЗАБРАТЬ";
    else if (["CASHING_OUT", "CASHED_OUT"].includes(carpet.status)) label.textContent = "ЗАБРАНО";
    else if (["CRASHING", "CRASHED"].includes(carpet.status)) label.textContent = "БУРЯ";
  ...
  if (button) {
    button.classList.toggle("accelerating", accelerating);
    button.classList.toggle("flying", carpet.status === "FLYING");
    button.classList.toggle("lost", ["CRASHING", "CRASHED"].includes(carpet.status));
  ...
  const history = $("#carpet-history-chips");
  if (history) {
    history.innerHTML = "";
...
function updateCarpetTrailEmitter() {
  ...
  if (jump > 96 || (carpetTrail.lastStatus !== state.carpet.status && state.carpet.status === "FLYING")) {
    resetCarpetTrail();
  }
  carpetTrail.lastX = point.x;
  carpetTrail.lastY = point.y;
  carpetTrail.lastStatus = state.carpet.status;
  const active = ["FLYING", "CASHED_OUT"].includes(state.carpet.status);
...
function addCarpetTrailParticle(now, power = 1) {
  const flying = state.carpet.status === "FLYING";
...
function renderCarpetTrailFrame(now = performance.now()) {
  ...
  const flying = activeTab && status === "FLYING";
...
function updateCarpetSparkOrigin(layer = getCarpetSparkLayer()) {
  const geometry = getCarpetHeroGeometry();
  const point = { x: geometry.engineX, y: geometry.engineY };
  if (layer) {
...
function renderCarpetSparkFrame(now = performance.now()) {
  const activeTab = document.body.dataset.activeTab === "solo";
  const status = state.carpet.status;
  const active = activeTab && (status === "READY" || status === "FLYING" || status === "CASHED_OUT");
  if (!active) {
...
function updateCarpetMagicSparks() {
  const active = ["READY", "FLYING", "CASHED_OUT"].includes(state.carpet.status);
...
function getTelegramProfile() {
  ...
  if (!user) {
    return {
      name: "Friedrich Kapitonovich",
      handle: "Telegram-профиль",
      initial: "F",
      photoUrl: "",
    };
  }

  const name = [user.first_name, user.last_name].filter(Boolean).join(" ") || "Игрок Mirage";
  return {
    name,
    handle: user.username ? `@${user.username}` : `Telegram ID ${user.id}`,
    initial: name.trim().charAt(0).toUpperCase() || "M",
    photoUrl: user.photo_url || "",
  };
}

function isAccountOperation(row) {
  return /покуп|попол|вывод|кошел|deposit|withdraw/i.test(row.label);
}

function renderAccountHistory(root, rows, emptyText) {
  if (!root) return;
  root.innerHTML = "";
  if (!rows.length) {
  ...
  rows.slice(0, 12).forEach((row) => {
    ...
    meta.textContent = "Текущая сессия";
    copy.append(label, meta);

    const amount = document.createElement("strong");
    amount.className = row.amount >= 0 ? "plus" : "minus";
    amount.textContent = `${row.amount >= 0 ? "+" : ""}${format(row.amount)}`;
...
function renderAccount() {
  ...
  const avatarImage = $("#account-avatar-image");
  const avatarFallback = $("#account-avatar-fallback");
  if (name) name.textContent = profile.name;
  if (handle) handle.textContent = profile.handle;
  if (avatarFallback) {
    avatarFallback.textContent = profile.initial;
    avatarFallback.hidden = Boolean(profile.photoUrl);
  }
  if (avatarImage) {
    avatarImage.hidden = !profile.photoUrl;
    if (profile.photoUrl && avatarImage.src !== profile.photoUrl) avatarImage.src = profile.photoUrl;
    avatarImage.onerror = () => {
  ...
  $$('[data-account-view]').forEach((button) => {
    button.classList.toggle("active", button.dataset.accountView === state.accountView);
  });
  $$('[data-account-panel]').forEach((panel) => {
    const active = panel.dataset.accountPanel === state.accountView;
    panel.classList.toggle("active", active);
    panel.setAttribute("aria-hidden", String(!active));
  });

  $$('[data-account-history]').forEach((button) => {
    button.classList.toggle("active", button.dataset.accountHistory === state.accountHistoryView);
  });

  const gamesRoot = $("#account-game-history");
  const transactionsRoot = $("#account-transaction-history");
  if (gamesRoot) gamesRoot.hidden = state.accountHistoryView !== "games";
  if (transactionsRoot) transactionsRoot.hidden = state.accountHistoryView !== "transactions";
  renderAccountHistory(gamesRoot, state.ledger.filter((row) => !isAccountOperation(row)), "История игр появится после первого запуска");
  renderAccountHistory(transactionsRoot, state.ledger.filter(isAccountOperation), "Операций пока нет");
}

function setAccountView(view) {
  if (!view || !["overview", "wallet", "history"].includes(view)) return;
  state.accountView = view;
  renderAccount();
  const scroll = $("#screen-profile .account-scroll");
  if (scroll) scroll.scrollTo({ top: 0, behavior: "smooth" });
}

function openAccountDeposit() {
  if (!switchTab("profile")) return false;
  state.accountView = "wallet";
  renderAccount();
  requestAnimationFrame(() => setAccountSheetOpen(true, "deposit"));
  return true;
}

function setAccountSheetOpen(open, type = "deposit") {
  const screen = $("#screen-profile");
  ...
  const icon = $("#account-sheet-icon");
  const title = $("#account-sheet-title");
  const copy = $("#account-sheet-copy");
  const action = $("#account-sheet-action");
  const rate = type === "rate";
  const deposit = type === "deposit";
  if (icon) icon.textContent = rate ? "i" : deposit ? "+" : "−";
  if (title) title.textContent = rate ? "КУРС РУБИНОВ" : deposit ? "ПОПОЛНЕНИЕ" : "ВЫВОД";
  if (copy) copy.textContent = rate
    ? "1 USDT = 100 рубинов. Пополнение в TON будет пересчитываться по актуальному курсу TON/USDT перед подтверждением операции."
    : deposit
      ? "Подключение платежей будет доступно после запуска защищенного сервера."
      : "Запросы на выплату будут доступны после запуска защищенного сервера.";
  if (action) {
    action.disabled = !rate;
    action.textContent = rate ? "ПОНЯТНО" : "НА ЭТАПЕ СЕРВЕРА";
  }
}
...
function getGameplayNavigationLock() {
  const homeActive = homeAutoRunning
    || homeManualRunning
    || homeManualAnimations.length > 0
    || homeManualPendingAnimations > 0
    || homeManualQueuedBalls > 0;
  if (homeActive) return { room: "home", reason: "Пирамида завершает игровое действие" };

  const carpet = state.carpet;
  if (carpet.status !== "READY" || carpet.autoRunning) {
    return { room: "solo", reason: "Ковёр завершает полёт" };
  }

  const slot = state.slot;
  const presentationActive = Boolean(slot.presentation?.type && slot.presentation.type !== "idle");
  const pharaohActive = slot.status !== "READY"
    || slot.roundFlowActive
    || slot.autoRunning
    || slot.bonusSessionActive
    || slot.bonusSessionFinalizing
    || presentationActive;
  if (pharaohActive) return { room: "pvp", reason: "Фараон завершает игровой процесс" };

  return null;
}

function syncNavigationLock() {
  const lock = getGameplayNavigationLock();
  const activeTab = document.body.dataset.activeTab || "home";
  document.body.classList.toggle("gameplay-navigation-locked", Boolean(lock));
  document.body.dataset.gameplayLock = lock?.room || "";

  $$(".bottom-nav button[data-tab]").forEach((button) => {
    const blocked = Boolean(lock) && button.dataset.tab !== activeTab;
    button.disabled = blocked;
    button.setAttribute("aria-disabled", blocked ? "true" : "false");
    if (blocked) button.title = lock.reason;
    else button.removeAttribute("title");
  });

  $$('[data-open-account-deposit], #player-profile-link, #carpet-profile-link, #pharaoh-profile-link').forEach((control) => {
...
function switchTab(tab) {
  if (!tab) return false;
  if (document.body.dataset.activeTab === tab) return true;
  if (getGameplayNavigationLock()) {
    syncNavigationLock();
    return false;
  }
  setRoomRulesOpen(null);
  setAccountSheetOpen(false);
  const shell = $(".app-shell");
  if (shell) shell.scrollTop = 0;
  document.body.dataset.activeTab = tab;
  window.mirageAudio?.setRoom(tab);
...
const homeBallImage = new Image();
const homeRewardCoinAtlas = new Image();
const homeMonkeyState = {
  left: { start: 0, until: 0, intensity: 0 },
  right: { start: 0, until: 0, intensity: 0 },
};
homeMonkeyImages.left.src = "./assets/plinko-monkey-guardian-v2.webp?v=telegram261";
homeMonkeyImages.right.src = "./assets/plinko-monkey-guardian-v2.webp?v=telegram261";
homeBallImage.src = "./assets/plinko-ball-premium-v2.png?v=telegram233";
homeRewardCoinAtlas.src = "./assets/plinko-reward-coin-atlas-v1.png?v=telegram261";
Object.values(homeMonkeyImages).forEach((image) => {
  image.onload = () => drawHomeBoard();
});
homeBallImage.onload = () => drawHomeBoard();
homeRewardCoinAtlas.onload = () => drawHomeBoard();
...
function getCanvasLayoutSize(canvas, fallbackWidth = 356, fallbackHeight = 360) {
  ...
  const width = Math.max(1, Math.round(canvas.clientWidth || canvas.offsetWidth || rect.width || fallbackWidth));
  const height = Math.max(1, Math.round(canvas.clientHeight || canvas.offsetHeight || rect.height || fallbackHeight));
  return { width, height };
}

function setupCanvasForDisplay(canvas, ctx) {
  const dpr = window.devicePixelRatio || 1;
  const { width, height } = getCanvasLayoutSize(canvas, canvas.width / dpr || 356, canvas.height / dpr || 360);
...
function getHomeBoardGeometry(width, rows, height = 420) {
  ...
  return { slotCount, pegTop, pegBottom, slotY, pegStep, pegGap, slotWidth, slotHeight, horizontalInset, centerX };
}
...
function getHomeMonkeyLayout(side, width, height) {
  ...
  return {
    ...
    width: drawWidth,
    height: drawHeight,
    mouthX: side === "left" ? left + drawWidth * 0.82 : left + drawWidth * 0.18,
    mouthY: top + drawHeight * 0.64,
  };
}
...
function drawHomeMonkeys(ctx, width, height, timestamp) {
  ["left", "right"].forEach((side) => {
    ...
    ctx.scale(side === "right" ? -scale : scale, scale);
...
function createHomeSparkBurst(slot, timestamp) {
  const { width, height } = getCanvasLayoutSize(homeCanvas, 377, 420);
...
function drawHomeWinEffects(ctx, timestamp) {
  ...
  for (let index = homeWinEffects.length - 1; index >= 0; index -= 1) {
    ...
    if (particle.type === "coin") {
      ...
      ctx.shadowColor = "rgba(255, 204, 64, 0.78)";
      ctx.shadowBlur = 5 + particle.size * 0.65;

      if (homeRewardCoinAtlas.complete && homeRewardCoinAtlas.naturalWidth > 0) {
        const cycleStep = Math.floor((((rotation / (Math.PI * 2)) % 1) + 1) % 1 * 4);
        const frame = cycleStep === 3 ? 1 : cycleStep;
        const diameter = particle.size * 2.85;
        ctx.drawImage(
          homeRewardCoinAtlas,
          frame * 256,
          0,
  ...
  function frame(timestamp) {
    ...
    for (let index = animations.length - 1; index >= 0; index -= 1) {
      ...
      if (frameState.done) {
        ...
        window.mirageAudio?.homeLand(animation.multiplier);
    ...
    if (activePegs.length > 0) window.mirageAudio?.homePeg(Math.min(1.6, activePegs.length / 3));
...
function resetCarpetRoundNow() {
  ...
  state.carpet.visualX = 0;
  state.carpet.visualY = 0;
  state.carpet.visualScale = 1;
  state.carpet.visualRotate = -8;
  state.carpet.visualProgress = 0;
  state.carpet.crashAnimationStartedAt = null;
  state.carpet.crashAnimationProgress = 0;
  state.carpet.cashoutAnimationStartedAt = null;
  state.carpet.cashoutAnimationProgress = 0;
  state.carpet.cashoutConfirmProgress = 0;
  state.carpet.cashoutOriginProgress = 0;
  state.carpet.cashoutOriginScale = 1;
...
function resetCarpetRound(delay = 980) {
  renderCarpet();
  setTimeout(resetCarpetRoundNow, delay);
}
...
function finishCarpetCrash() {
  ...
  state.games += 1;
  carpet.crashOriginX = carpet.visualX;
  carpet.crashOriginY = carpet.visualY;
  carpet.crashOriginScale = carpet.visualScale;
  carpet.crashOriginRotate = carpet.visualRotate;
  carpet.crashOriginProgress = carpet.visualProgress;

  const targetProgress = Math.min(1, carpet.visualProgress + 0.025);
  const targetPoint = getCarpetPointOnPath(targetProgress);
  const targetScale = 1 - targetProgress * 0.02;
  const targetTranslation = getCarpetTranslationForEngine(targetPoint, targetScale, targetPoint.rotate);
  carpet.crashTargetX = targetTranslation.x;
  carpet.crashTargetY = targetTranslation.y;
  carpet.crashTargetScale = targetScale;
  carpet.crashTargetRotate = targetPoint.rotate;
  carpet.crashAnimationStartedAt = performance.now();
  carpet.crashAnimationProgress = 0;
  carpet.status = "CRASHING";
  window.mirageAudio?.carpetCrash();
  render();
  carpet.animationFrameId = requestAnimationFrame(tickCarpetCrashAnimation);
}

function tickCarpetCrashAnimation(timestamp) {
  const carpet = state.carpet;
  if (!["CRASHING", "CRASHED"].includes(carpet.status) || !carpet.crashAnimationStartedAt) return;
  const elapsed = Math.max(0, timestamp - carpet.crashAnimationStartedAt);
  const impactProgress = clampUnit(elapsed / CARPET_CRASH_IMPACT_MS);
  const dissolveProgress = clampUnit((elapsed - CARPET_CRASH_IMPACT_MS) / CARPET_CRASH_DISSOLVE_MS);
  const movementProgress = elapsed <= CARPET_CRASH_IMPACT_MS ? impactProgress : 1;
  const movementEase = 1 - Math.pow(1 - movementProgress, 3);

  carpet.visualX = carpet.crashOriginX + (carpet.crashTargetX - carpet.crashOriginX) * movementEase;
  carpet.visualY = carpet.crashOriginY + (carpet.crashTargetY - carpet.crashOriginY) * movementEase;
  carpet.visualScale = carpet.crashOriginScale + (carpet.crashTargetScale - carpet.crashOriginScale) * movementEase;
  carpet.visualRotate = carpet.crashOriginRotate + (carpet.crashTargetRotate - carpet.crashOriginRotate) * movementEase;
  carpet.visualProgress = carpet.crashOriginProgress + (Math.min(1, carpet.crashOriginProgress + 0.025) - carpet.crashOriginProgress) * movementEase;
  carpet.crashAnimationProgress = dissolveProgress;

  const dissolveFinishedAt = CARPET_CRASH_IMPACT_MS + CARPET_CRASH_DISSOLVE_MS;
  if (elapsed >= dissolveFinishedAt && carpet.status !== "CRASHED") {
    carpet.status = "CRASHED";
  }
  renderCarpet();

  if (elapsed < dissolveFinishedAt + CARPET_CRASH_RESULT_HOLD_MS) {
    carpet.animationFrameId = requestAnimationFrame(tickCarpetCrashAnimation);
    return;
  }
...
function cashoutCarpetRound() {
  ...
  const originPose = getCarpetVisualPose(carpet);
  if (carpet.animationFrameId) cancelAnimationFrame(carpet.animationFrameId);
  const locked = carpetEngine.lockMultiplier(carpet.multiplier);
  const payout = carpetEngine.getPayout(carpet.stake, locked);
  carpet.animationFrameId = null;
  carpet.status = "CASHING_OUT";
  carpet.visualX = originPose.x;
  carpet.visualY = originPose.y;
  carpet.visualScale = originPose.scale;
  carpet.visualRotate = originPose.rotate;
  carpet.visualProgress = originPose.pathProgress;
  carpet.cashoutOriginProgress = originPose.pathProgress;
  carpet.cashoutOriginScale = originPose.scale;
  carpet.cashoutAnimationStartedAt = performance.now();
  carpet.cashoutAnimationProgress = 0;
  carpet.cashoutConfirmProgress = 0;
  carpet.cashedOutMultiplier = locked;
  carpet.payout = payout;
  window.mirageAudio?.carpetCashout();
  carpet.protectedRoundsPlayed += 1;
  ...
  state.games += 1;
  render();
  carpet.animationFrameId = requestAnimationFrame(tickCarpetCashoutAnimation);
}

function tickCarpetCashoutAnimation(timestamp) {
  const carpet = state.carpet;
  if (!["CASHING_OUT", "CASHED_OUT"].includes(carpet.status) || !carpet.cashoutAnimationStartedAt) return;
  const elapsed = Math.max(0, timestamp - carpet.cashoutAnimationStartedAt);
  const confirmProgress = clampUnit(elapsed / CARPET_CASHOUT_CONFIRM_MS);
  const exitProgress = clampUnit((elapsed - CARPET_CASHOUT_CONFIRM_MS) / CARPET_CASHOUT_EXIT_MS);
  const movementEase = easeInOutCubic(exitProgress);
  const targetPathProgress = Math.min(
    0.999,
    Math.max(CARPET_CASHOUT_MIN_EXIT_PROGRESS, carpet.cashoutOriginProgress + 0.08),
  );
  const pathProgress = carpet.cashoutOriginProgress
    + (targetPathProgress - carpet.cashoutOriginProgress) * movementEase;
  const point = getCarpetPointOnPath(pathProgress);
  const targetScale = carpet.cashoutOriginScale * 0.72;
  const scale = carpet.cashoutOriginScale + (targetScale - carpet.cashoutOriginScale) * movementEase;
  const translation = getCarpetTranslationForEngine(point, scale, point.rotate);

  carpet.visualX = translation.x;
  carpet.visualY = translation.y;
  carpet.visualScale = scale;
  carpet.visualRotate = point.rotate;
  carpet.visualProgress = pathProgress;
  carpet.cashoutAnimationProgress = exitProgress;
  carpet.cashoutConfirmProgress = confirmProgress;

  const exitFinishedAt = CARPET_CASHOUT_CONFIRM_MS + CARPET_CASHOUT_EXIT_MS;
  if (elapsed >= exitFinishedAt) carpet.status = "CASHED_OUT";
  renderCarpet();

  if (elapsed < exitFinishedAt + CARPET_CASHOUT_RESULT_HOLD_MS) {
    carpet.animationFrameId = requestAnimationFrame(tickCarpetCashoutAnimation);
    return;
  }
...
function tickCarpetRound(timestamp) {
  ...
  window.mirageAudio?.updateCarpetFlight(carpet.multiplier);
  ...
  if (new URLSearchParams(window.location.search).get("debugCarpetCashout") === "1" && carpet.multiplier >= 1.35) {
    cashoutCarpetRound();
    return;
  }
...
function startCarpetRound() {
  ...
  if (new URLSearchParams(window.location.search).get("debugCarpetCrash") === "1") {
    round.crashPoint = 1.35;
  }
  const startedAt = performance.now();
  ...
  carpet.roundStartedAt = startedAt;
  carpet.visualX = 0;
  carpet.visualY = 0;
  carpet.visualScale = 1;
  carpet.visualRotate = -8;
  carpet.visualProgress = 0;
  carpet.crashAnimationStartedAt = null;
  carpet.crashAnimationProgress = 0;
  carpet.cashoutAnimationStartedAt = null;
  carpet.cashoutAnimationProgress = 0;
  carpet.cashoutConfirmProgress = 0;
  carpet.cashoutOriginProgress = 0;
  carpet.cashoutOriginScale = 1;
  carpet.cashedOutMultiplier = null;
  carpet.pendingCashout = false;
  carpet.payout = 0;
  window.mirageAudio?.startCarpetFlight();
...
function triggerCarpetMainButton() {
  if (["READY", "FLYING"].includes(state.carpet.status)) pulseCarpetMainButton();
...
function clearSlotBonusSessionTimer() {
  const slot = state.slot;
  if (!slot.bonusSessionTimer) return;
  window.clearTimeout(slot.bonusSessionTimer);
  slot.bonusSessionTimer = null;
}

function clearSlotBonusFinalTimer() {
  const slot = state.slot;
  if (!slot.bonusSessionFinalTimer) return;
  window.clearTimeout(slot.bonusSessionFinalTimer);
  slot.bonusSessionFinalTimer = null;
}

function beginSlotBonusSession(spins) {
  const slot = state.slot;
  clearSlotBonusSessionTimer();
  clearSlotBonusFinalTimer();
  slot.bonusSessionActive = true;
  slot.bonusSessionRunning = true;
  slot.bonusSessionFinalizing = false;
  slot.bonusSessionInitialSpins = spins;
  slot.bonusSessionPlayed = 0;
  slot.bonusSessionTotalWin = 0;
  slot.freeSpinMultiplierBank = 0;
  slot.multiplierMeterPrevious = 1;
  slot.multiplierMeterDelta = 0;
  slot.multiplierMeterPreview = 1;
  slot.multiplierMeterFormula = "";
  slot.multiplierMeterMode = "bonus";
}

function cancelSlotBonusSession() {
  const slot = state.slot;
  if (!slot.bonusSessionActive && !slot.bonusSessionRunning) return false;
  clearSlotBonusSessionTimer();
  slot.bonusSessionRunning = false;
  slot.message = slot.freeSpins > 0 ? `Бонус на паузе: ${slot.freeSpins} free spins` : "Бонус завершен";
  slot.reaction = "idle";
  renderPharaoh();
  return true;
}

function finishSlotBonusSession(delay = 500) {
  const slot = state.slot;
  if (!slot.bonusSessionActive || slot.freeSpins > 0 || slot.status !== "READY") return false;
  const totalWin = slot.bonusSessionTotalWin;
  const played = slot.bonusSessionPlayed;
  clearSlotBonusSessionTimer();
  clearSlotBonusFinalTimer();
  slot.bonusSessionActive = false;
  slot.bonusSessionRunning = false;
  slot.bonusSessionFinalizing = totalWin > 0;
  slot.bonusSessionInitialSpins = 0;
  slot.bonusSessionPlayed = 0;
  slot.bonusSessionTotalWin = 0;
  if (totalWin <= 0) slot.freeSpinMultiplierBank = 0;
  slot.multiplierMeterPrevious = Math.max(1, slot.freeSpinMultiplierBank || 0);
  slot.multiplierMeterDelta = 0;
  slot.multiplierMeterPreview = 1;
  slot.multiplierMeterFormula = "";
  slot.multiplierMeterMode = totalWin > 0 ? "final" : "bonus";
  slot.message = totalWin > 0 ? `Итог бонуса ${format(totalWin)} 💎` : "Бонус без выигрыша";
  slot.reaction = totalWin > 0 ? "win" : "idle";
  renderPharaoh();
  if (totalWin > 0) {
    const finalPresentation = {
      type: totalWin / Math.max(1, slot.stake) >= 30 ? "mega" : "free-win",
      title: "ИТОГ БОНУСА",
      value: "",
      win: totalWin,
      ruby: true,
      note: formatSlotSpinCount(played || SLOT_CONFIG.bonusBuy.freeSpins),
    };
    const finalDuration = getSlotPresentationDuration(finalPresentation, 3900);
    window.setTimeout(() => {
      showSlotPresentation(finalPresentation, 3900);
      renderSlotPresentation(slot);
    }, delay);
    slot.bonusSessionFinalTimer = window.setTimeout(() => {
      slot.bonusSessionFinalizing = false;
      slot.bonusSessionFinalTimer = null;
      slot.freeSpinMultiplierBank = 0;
      slot.multiplierMeterPrevious = 1;
      ...
      slot.multiplierMeterMode = "bonus";
      renderPharaoh();
    }, delay + finalDuration + 120);
  }
  return true;
}

function scheduleSlotBonusSessionNext(delay = 760) {
  const slot = state.slot;
  if (!slot.bonusSessionActive || !slot.bonusSessionRunning) return false;
  clearSlotBonusSessionTimer();
  if (slot.status !== "READY") return false;
  if (slot.freeSpins <= 0) return finishSlotBonusSession(delay);
  slot.bonusSessionTimer = window.setTimeout(() => {
    slot.bonusSessionTimer = null;
    if (slot.status === "READY" && slot.bonusSessionActive && slot.bonusSessionRunning && slot.freeSpins > 0) {
      playPharaohSlot({ bonusSessionStep: true });
    }
  }, delay);
  return true;
}

function continueSlotFlowsAfterRound({ bonusBuy = false, isFreeSpin = false, presentation = null, duration = 0 } = {}) {
  const slot = state.slot;
  if (slot.bonusSessionActive && (bonusBuy || isFreeSpin || slot.freeSpins > 0)) {
    const delay = presentation ? Math.max(duration + 520, 1850) : 760;
    scheduleSlotBonusSessionNext(delay);
    return;
  }
...
function triggerPharaohMainButton() {
  const slot = state.slot;
  if (slot.bonusSessionRunning) {
    cancelSlotBonusSession();
    return;
  }
  if (slot.autoRunning) {
    cancelSlotAutoRuns();
  ...
  if (slot.bonusSessionActive && slot.freeSpins > 0) {
    slot.bonusSessionRunning = true;
    slot.message = `Бонус продолжается: ${slot.freeSpins} free spins`;
    renderPharaoh();
    playPharaohSlot({ bonusSessionStep: true });
    return;
  }
...
function recoverPharaohSpin(message = "Спин сброшен") {
  const slot = state.slot;
  window.mirageAudio?.pharaohSpinSettled();
  clearSlotBonusSessionTimer();
  clearSlotBonusFinalTimer();
  slot.status = "READY";
  slot.roundFlowActive = false;
  slot.autoRunning = false;
  slot.autoRunsRemaining = 0;
  slot.bonusSessionRunning = false;
  slot.bonusSessionFinalizing = false;
...
function settlePharaohRoundResult(result) {
  ...
  if (result.freeSpinsAwarded > 0) {
    const spinsAlreadyCommitted = slot.bonusSessionActive
      ? slot.bonusSessionPlayed + result.freeSpinsPlayed + slot.freeSpins
      : 0;
    const availableSpins = Math.max(0, SLOT_CONFIG.maxBonusSpinsPerRound - spinsAlreadyCommitted);
    result.freeSpinsAwarded = Math.min(result.freeSpinsAwarded, availableSpins);
  }

  if (result.freeSpinsAwarded > 0) {
    slot.freeSpins += result.freeSpinsAwarded;
    if (!slot.bonusSessionActive && result.freeSpinsAwarded > 0) {
      beginSlotBonusSession(result.freeSpinsAwarded);
    } else if (slot.bonusSessionActive) {
      slot.bonusSessionInitialSpins += result.freeSpinsAwarded;
    }
    slot.message = result.bonusGranted
      ? `Бонус открыт: ${result.freeSpinsAwarded} free spins`
      : `Дар джина: +${result.freeSpinsAwarded} free spins`;
    showSlotScatterAward();
  } else if (result.totalWin > 0) {
    slot.message = `Выигрыш ${format(result.totalWin)} 💎`;
  } else {
    slot.message = "Без выигрыша";
  }
  slot.reaction = result.freeSpinsAwarded > 0 ? "free" : result.totalWin > 0 ? "win" : "idle";
  slot.spinPreviewGrid = null;
  slot.spinVisualMode = "idle";
  ...
  if (slot.bonusSessionActive && result.freeSpinsPlayed > 0) {
    slot.bonusSessionPlayed += result.freeSpinsPlayed;
    slot.bonusSessionTotalWin += result.totalWin;
  }
  if (result.freeSpinsPlayed > 0) {
    slot.freeSpinMultiplierBank = result.freeMultiplierBank || 0;
  }
  if (!slot.bonusSessionActive && slot.freeSpins <= 0 && result.freeSpinsPlayed > 0) {
    slot.freeSpinMultiplierBank = 0;
  }
  slot.explanation = result.totalWin > 0 ? buildSlotResultExplanation(result) : null;
...
async function playPharaohSlot({ bonusBuy = false, autoStep = false, bonusSessionStep = false } = {}) {
  const slot = state.slot;
  if (slot.status !== "READY") return;
  ...
  if (bonusSessionStep && (!slot.bonusSessionActive || !slot.bonusSessionRunning || slot.freeSpins <= 0)) {
    finishSlotBonusSession();
    return;
  }
  ...
  if (bonusBuy && bonusCost > getSlotBonusBuyLimit()) {
    slot.message = "Покупка выше лимита пула";
  ...
  try {
    ...
    if (cost > 0) {
      ...
      chargedCost = cost;
      addLedger(bonusBuy ? "Фараон: покупка free spins" : "Фараон: спин", -cost);
    }
    ...
    window.mirageAudio?.pharaohSpinStart(bonusBuy || isFreeSpin);
    renderPharaoh();
    ...
    const result = createSlotRoundResult(slot.stake, {
      ...
      freeMultiplierBank: isFreeSpin ? slot.freeSpinMultiplierBank : 0,
    });
    ...
    if (ultraLiteRenderer && !shouldUsePharaohCanvasRenderer()) {
      ...
      const requestedPresentationDuration = bonusBuy ? 3600 : 3200;
      const presentationDuration = getSlotRoundPresentationDuration(presentation, requestedPresentationDuration);
      if (presentation) await showSlotPresentationAfterSettle(presentation, {
        duration: presentationDuration,
        delay: 180,
      });
      roundSettled = true;
      render();
      continueSlotFlowsAfterRound({ bonusBuy, isFreeSpin, presentation, duration: presentationDuration });
      return;
    }

    const freeSpinBank = slot.freeSpins;
    ...
    if (canvasRenderer) {
      ...
      await pharaohCanvasRenderer.playResult(
        ...
        {
          ...(ultraLiteRenderer
            ? { reel: 1020, drop: 220, win: 560, cascade: 1020, empty: 70 }
            : liteRenderer ? { reel: 1100, drop: 280, win: 700, cascade: 1100, empty: 90 } : { reel: 1180, win: 780, cascade: 1180 }),
          onStep: (step) => {
            if (slot.spinRunId !== runId) return;
            ...
            if (step.phase === "free") {
              const previousBank = Math.max(1, slot.freeSpinMultiplierBank || 0);
              slot.freeSpins = freeSpinBank;
              slot.freeSpinMultiplierBank = step.multiplierBank || slot.freeSpinMultiplierBank;
              showSlotMultiplierIncrease(previousBank, Math.max(1, slot.freeSpinMultiplierBank || 0), step.multiplierAdded || 0);
            }
            showSlotMultiplierStep(step, step.phase);
            if ((step.multiplierAdded || 0) > 0) window.mirageAudio?.pharaohMultiplier(step.multiplierAdded);
            if (step.wins.length > 0) {
              window.mirageAudio?.pharaohCascade(step.win);
              const sessionFreeIndex = slot.bonusSessionActive ? slot.bonusSessionPlayed + step.freeSpinIndex : step.freeSpinIndex;
              slot.message = step.phase === "free" ? `Free ${sessionFreeIndex}: +${format(step.win)} 💎` : `Каскад: +${format(step.win)} 💎`;
              slot.explanation = buildSlotStepExplanation(step);
            }
          ...
          onReelStop: (column) => window.mirageAudio?.pharaohReelStop(column),
          onReelSettled: () => window.mirageAudio?.pharaohSpinSettled(),
          onCascadeDrop: () => window.mirageAudio?.pharaohCascadeDrop(),
        }
      );
      if (slot.spinRunId !== runId) return;
      slot.winningKeys = new Set();
      ...
      for (let previewIndex = 0; previewIndex < previewPasses; previewIndex += 1) {
        ...
        renderPharaoh();
        window.mirageAudio?.pharaohReelStop(previewIndex);
        await nextAnimationFrame();
      }
      window.mirageAudio?.pharaohSpinSettled();
      ...
      for (const [index, step] of displaySteps.entries()) {
        ...
        const sessionFreeIndex = slot.bonusSessionActive ? slot.bonusSessionPlayed + step.freeSpinIndex : step.freeSpinIndex;
        const phaseLabel = step.phase === "free" ? `Free ${sessionFreeIndex}` : `Каскад ${index + 1}`;
        if (step.phase === "free") {
          const previousBank = Math.max(1, slot.freeSpinMultiplierBank || 0);
          slot.freeSpins = freeSpinBank;
          slot.freeSpinMultiplierBank = step.multiplierBank || slot.freeSpinMultiplierBank;
        ...
        showSlotMultiplierStep(step, step.phase);
        if ((step.multiplierAdded || 0) > 0) window.mirageAudio?.pharaohMultiplier(step.multiplierAdded);
        if (step.wins.length > 0) window.mirageAudio?.pharaohCascade(step.win);
        ...
        slot.explanation = step.wins.length > 0 ? buildSlotStepExplanation(step) : slot.explanation;
        ...
        if (step.nextGrid && step.wins.length > 0) window.mirageAudio?.pharaohCascadeDrop();
      }
    }
    ...
    if (presentation) await showSlotPresentationAfterSettle(presentation, {
      duration: presentationDuration,
      delay: result.totalWin > 0 ? ultraLiteRenderer ? 360 : liteRenderer ? 300 : 260 : 120,
    });
    roundSettled = true;
    render();
    continueSlotFlowsAfterRound({ bonusBuy, isFreeSpin, presentation, duration: presentationDuration });
    ...
    if (slot.spinRunId === runId || slot.status === "READY") {
      slot.roundFlowActive = false;
      syncNavigationLock();
    }
  }
...
function initDebugSlotWinCeremony() {
  const params = new URLSearchParams(window.location.search);
  if (params.get("debugBonusFinal") === "1") {
    window.setTimeout(() => {
      switchTab("pvp");
      const slot = state.slot;
      slot.status = "READY";
      slot.bonusSessionActive = true;
      slot.bonusSessionRunning = false;
      slot.bonusSessionPlayed = 15;
      slot.bonusSessionTotalWin = 360;
      slot.freeSpins = 0;
      slot.freeSpinMultiplierBank = 5;
      slot.multiplierMeterMode = "bonus";
      renderPharaoh();
      window.setTimeout(() => finishSlotBonusSession(300), 850);
    }, 900);
    return;
  }
  if (params.get("debugMultiplierMeter") === "1") {
    window.setTimeout(() => {
      switchTab("pvp");
      const slot = state.slot;
      slot.bonusSessionActive = true;
      ...
      slot.freeSpinMultiplierBank = 5;
      slot.multiplierMeterMode = "bonus";
      slot.multiplierMeterFormula = "18 × x5 → 90";
      slot.multiplierMeterDelta = 3;
      renderPharaoh();
      const meter = $("#slot-multiplier-meter");
      if (meter) meter.classList.add("collecting");
      window.setTimeout(() => {
        if (meter) meter.classList.remove("collecting");
        slot.multiplierMeterFormula = "";
        slot.multiplierMeterDelta = 0;
        renderPharaoh();
      }, 2100);
  ...
  if (params.get("debugMultiplier") === "1") {
    window.setTimeout(() => {
      switchTab("pvp");
      const slot = state.slot;
      slot.grid = [
        ["ankh", "scarab", "blue", "multi:5", "coin", "ruby"],
        ["coin", "green", "ankh", "scroll", "scarab", "coin"],
        ["blue", "scarab", "lamp", "green", "ruby", "ankh"],
        ["scroll", "coin", "blue", "lamp", "green", "ruby"],
        ["ankh", "lamp", "coin", "blue", "scroll", "scarab"],
      ];
      slot.winningKeys = new Set();
      slot.reaction = "idle";
      slot.message = "Множитель на поле";
      renderPharaoh();
    }, 900);
    return;
  }
  const debugFreeSpins = params.get("debugFreeSpins");
  if (debugFreeSpins === "1" || debugFreeSpins === "auto") {
    window.setTimeout(() => {
      switchTab("pvp");
      const slot = state.slot;
      slot.grid = [
        ["scatter", "ruby", "scatter", "lamp", "blue", "scatter"],
        ["coin", "green", "ankh", "scroll", "scatter", "coin"],
        ["blue", "scarab", "lamp", "green", "ruby", "ankh"],
        ["scroll", "coin", "blue", "lamp", "green", "ruby"],
        ["ankh", "lamp", "coin", "blue", "scroll", "scarab"],
      ];
      slot.winningKeys = new Set(["0-0", "0-2", "0-5", "1-4"]);
      slot.reaction = "free";
      slot.message = "Дар джина открыт";
      slot.freeSpins = Math.max(slot.freeSpins, 10);
      if (debugFreeSpins === "auto") beginSlotBonusSession(slot.freeSpins);
      renderPharaoh();
      updateSlotGridWinSignal($("#slot-grid"), slot.winningKeys.size, true);
      showSlotScatterAward();
      showSlotPresentation({
        type: "free",
        title: "ДАР ДЖИНА",
        value: "10 FREE SPINS",
        note: "4 scatter",
      ...
      renderSlotPresentation(slot);
      if (debugFreeSpins === "auto") scheduleSlotBonusSessionNext(4700);
    }, 900);
    return;
  }
  const type = params.get("debugWinCeremony");
  if (!SLOT_WIN_CEREMONY_TIERS.has(type)) return;
  const debugWin = Math.max(100, Number(params.get("debugWinAmount")) || (type === "legend" ? 1800 : type === "mega" ? 620 : 180));
  window.setTimeout(() => {
    switchTab("pvp");
    const slot = state.slot;
    slot.totalWin = debugWin;
    slot.message = `${SLOT_WIN_CEREMONY_LABELS[type] || "ВЫИГРЫШ"} ${format(debugWin)} 💎`;
    slot.reaction = type === "big" ? "big" : type === "mega" ? "mega" : "legend";
    renderPharaoh();
    showSlotPresentation({
      type,
      title: SLOT_WIN_CEREMONY_LABELS[type] || "ВЫИГРЫШ",
      value: "",
      win: debugWin,
      ruby: true,
      note: type === "legend" ? "100x+" : type === "mega" ? "30x+" : "10x+",
    }, type === "legend" ? 4200 : type === "mega" ? 3600 : 3100);
    renderSlotPresentation(slot);
  }, 900);
}
...
function initEvents() {
  $$('[data-room-rules-open]').forEach((button) => {
    const openRules = (event) => {
      event.preventDefault();
      event.stopPropagation();
      setRoomRulesOpen(button.dataset.roomRulesOpen);
    };
    button.addEventListener('click', openRules);
  });

  $$('[data-room-rules-close], [data-room-rules-backdrop]').forEach((button) => {
  ...
  $$('[data-account-open-wallet]').forEach((button) => {
    button.addEventListener("click", () => setAccountView("wallet"));
  });

  $$('[data-account-history]').forEach((button) => {
    button.addEventListener("click", () => {
      pulseChoice(button);
      state.accountHistoryView = button.dataset.accountHistory;
      renderAccount();
    });
  });

  $$('[data-open-account-deposit]').forEach((button) => {
    button.addEventListener("click", openAccountDeposit);
  });

  $$('[data-account-transaction]').forEach((button) => {
    button.addEventListener("click", () => setAccountSheetOpen(true, button.dataset.accountTransaction));
  });

  const accountRateInfo = $("#account-rate-info");
  if (accountRateInfo) accountRateInfo.addEventListener("click", () => setAccountSheetOpen(true, "rate"));

  const accountSheetBackdrop = $("#account-sheet-backdrop");
  if (accountSheetBackdrop) accountSheetBackdrop.addEventListener("click", () => setAccountSheetOpen(false));
  const accountSheetClose = $("#account-sheet-close");
  if (accountSheetClose) accountSheetClose.addEventListener("click", () => setAccountSheetOpen(false));
  const accountSheetAction = $("#account-sheet-action");
  if (accountSheetAction) accountSheetAction.addEventListener("click", () => {
    if (!accountSheetAction.disabled) setAccountSheetOpen(false);
  });
  ...
  if (slotBonusBuy) {
    slotBonusBuy.addEventListener("click", () => {
      ...
      if (!SLOT_CONFIG.bonusBuy.enabled || state.slot.status !== "READY" || state.slot.freeSpins > 0 || bonusCost > state.balance || bonusCost > getSlotBonusBuyLimit()) return;
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (state.rulesRoom) setRoomRulesOpen(null);
    setAccountSheetOpen(false);
  });
...
initEvents();
render();
initDebugSlotWinCeremony();
initPharaohEconomyAudit();
...
window.addEventListener("resize", () => {
  ...
  window.requestAnimationFrame(() => window.requestAnimationFrame(() => positionSlotMultiplierMeter()));
});
