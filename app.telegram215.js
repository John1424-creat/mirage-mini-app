...
const SLOT_CONFIG = {
  ...
  symbols: [
    ...
    { id: "scatter", label: "☀", name: "Дар джина", tier: "scatter", weight: 1.05, color: "#f4ccff", icon: "./assets/pharaoh-icons-lite/genie-scatter-v1.png" },
...
function createSlotRoundResult(stake, { bonusBuy = false, freeSpinOnly = false } = {}) {
  ...
  const spin = createSlotSpinResult(stake, freeSpinOnly ? "free" : "base");
  if (freeSpinOnly) {
    freeSpinsPlayed += 1;
...
function renderPharaoh() {
  ...
  const bonusAvailable = SLOT_CONFIG.bonusBuy.enabled && slot.status === "READY" && !slot.autoRunning && !slot.bonusSessionActive && slot.freeSpins === 0 && bonusCost <= state.balance;
  if (bonus) {
    bonus.disabled = !bonusAvailable;
    bonus.classList.toggle("available", bonusAvailable);
    bonus.textContent = SLOT_CONFIG.bonusBuy.enabled ? "15 FREE SPINS" : "БОНУС СКОРО";
    bonus.title = SLOT_CONFIG.bonusBuy.enabled ? `Купить 15 free spins за ${format(bonusCost)} кристаллов` : "Бонус будет включен позже";
  }
  ...
  if (bonusNote) {
    bonusNote.textContent = `Цена покупки: ${format(slot.stake)} × ${SLOT_CONFIG.bonusBuy.costMultiplier} = ${format(bonusCost)} 💎. После подтверждения запускается серия из ${SLOT_CONFIG.bonusBuy.freeSpins} free spins.`;
  }
