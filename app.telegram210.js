...
const SLOT_WIN_TREASURE_PARTICLES = {
  ...
  sun: "./assets/pharaoh-icons-lite/sun-scatter.webp",
...
function getSlotWinPresentation(result, { bonusBuy = false, isFreeSpin = false } = {}) {
  ...
  if (multiplier >= 100) {
    return { type: "legend", title: "ЛЕГЕНДАРНО", value: "", win: result.totalWin, ruby: true, note };
  }
  if (multiplier >= 30) {
    return { type: "mega", title: "МЕГА ВЫИГРЫШ", value: "", win: result.totalWin, ruby: true, note };
  }
  if (multiplier >= 10) {
    return { type: "big", title: "БОЛЬШОЙ ВЫИГРЫШ", value: "", win: result.totalWin, ruby: true, note };
  }
