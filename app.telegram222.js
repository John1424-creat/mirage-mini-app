...
const SLOT_CONFIG = {
  ...
  symbols: [
    ...
    { id: "multi", label: "×", name: "Множитель", tier: "multi", weight: 0.55, color: "#ff73d5", icon: "./assets/pharaoh-icons-premium/multiplier-orb-v1.webp" },
  ],
  payouts: {
...
const STARTUP_ASSETS = [
  ...
  "./assets/pharaoh-icons-premium/multiplier-orb-v1.webp",
...
const SLOT_WIN_TREASURE_PARTICLES = {
  ...
  orb: "./assets/pharaoh-icons-premium/multiplier-orb-v1.webp",
};
...
class PharaohCanvasRenderer {
  ...
  drawSymbol(symbolId, x, y, w, h, options = {}) {
    ...
    const isScatter = symbol.id === "scatter";
    const isFeature = symbol.tier === "scatter" || symbol.tier === "multi";
    const iconRatio = symbol.tier === "scatter" ? 0.88 : symbol.tier === "multi" ? 0.82 : symbol.tier === "high" ? 0.84 : 0.82;
    const iconSize = Math.min(w, h) * iconRatio * scale;
    const iconCx = symbol.id === "multi" ? cx - iconSize * (7 / 384) : cx;
    const iconCy = symbol.id === "multi" ? cy + iconSize * (7.5 / 384) : cy;
    ctx.save();
    ctx.globalAlpha = alpha;
...
function drawSlotWinTreasureParticle(ctx, x, y, size, rotation, alpha, kind = "coin", depth = 1) {
  if (kind === "dust") {
    ...
    ctx.globalCompositeOperation = "lighter";
    const glow = ctx.createRadialGradient(0, 0, 0, 0, 0, size * 0.72);
    glow.addColorStop(0, "rgba(255, 245, 178, 0.95)");
    glow.addColorStop(0.36, "rgba(255, 190, 72, 0.42)");
  ...
  ctx.globalAlpha = alpha;
  ctx.globalCompositeOperation = "source-over";
  ctx.shadowColor = kind === "ruby" ? "rgba(255, 97, 215, 0.74)" : "rgba(255, 224, 115, 0.68)";
  ctx.shadowBlur = Math.max(6, size * 0.42 * depth);
  if (image.complete && image.naturalWidth > 0) {
    const wobble = kind === "coin" ? 0.86 + Math.abs(Math.sin(rotation)) * 0.16 : 1;
    ctx.scale(wobble, 1);
    ctx.drawImage(image, -size / 2, -size / 2, size, size);
  } else {
    const glow = ctx.createRadialGradient(0, 0, 0, 0, 0, size * 0.46);
    glow.addColorStop(0, kind === "ruby" ? "#fff0ff" : "#fff4b8");
    glow.addColorStop(0.48, kind === "ruby" ? "#ff55cd" : "#ffd24d");
