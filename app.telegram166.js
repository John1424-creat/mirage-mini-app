...
class PharaohCanvasRenderer {
  ...
  async animateReel(targetGrid, context = "base", duration = 760) {
    ...
    for (let col = 0; col < cols; col += 1) {
      for (let row = 0; row < rows; row += 1) {
        incoming.push({
          ...
          row,
          col,
          delay: row * 0.07 + col * 0.008,
          distance: rowStride * (rows + 1.25 + (col % 2) * 0.18),
        });
      }
    }
    const outgoing = [];
      ...
      for (let row = 0; row < rows; row += 1) {
        for (let col = 0; col < cols; col += 1) {
          ...
          outgoing.push({
            ...
            row,
            col,
            delay: row * 0.062 + col * 0.006,
            distance: rowStride * (rows + 1.45 + (col % 2) * 0.16),
          });
        }
      }
    }
    ...
    await this.animate(duration, (progress) => {
      this.drawBackground();
      const ctx = this.ctx;
      if (outgoing.length && progress < 0.82) {
        outgoing.forEach((sprite) => {
          const local = Math.max(0, Math.min(1, (progress - sprite.delay) / 0.52));
          if (local <= 0) {
      ...
      incoming.forEach((sprite) => {
        const entryStart = outgoing.length ? 0.48 : 0.03;
        const delay = entryStart + sprite.delay * (outgoing.length ? 0.72 : 1);
        const local = Math.max(0, Math.min(1, (progress - delay) / Math.max(0.24, 1 - delay)));
        if (local <= 0) return;
