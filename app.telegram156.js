...
class PharaohCanvasRenderer {
  ...
  async animateCascadeTransition(fromGrid, winningKeys, nextGrid, duration = 560) {
    ...
    await this.animate(duration, (progress) => {
      const vanishProgress = Math.min(1, progress / 0.34);
      const dropProgress = Math.max(0, Math.min(1, (progress - 0.16) / 0.84));
      const dropEase = easeOut(dropProgress);
      const bounce = Math.sin(dropEase * Math.PI) * Math.max(1.4, cellH * 0.045);
      const shimmer = 0.5 + Math.sin(progress * Math.PI * 5) * 0.5;

      this.drawBackground();
      if (progress < 0.42) {
        if (!this.drawWinGroupNetwork([{ keys: [...keys] }], Math.min(1, progress / 0.42))) {
          this.drawWinLinks(keys, Math.min(1, progress / 0.42));
        }
      }
      ...
      sprites.survivors.forEach((sprite) => {
        ...
        const y = fromY + (toY - fromY) * dropEase + (sprite.fromRow !== sprite.toRow ? bounce : 0);
        this.drawSymbol(sprite.symbolId, x, y, cellW, cellH, {
          alpha: 0.88 + dropEase * 0.12,
          scale: 0.99 + dropEase * 0.01,
        });
      });

      sprites.incoming.forEach((sprite, index) => {
        const columnDelay = sprite.col * 0.025 + index * 0.01;
        const local = Math.max(0, Math.min(1, (dropProgress - columnDelay) / Math.max(0.45, 1 - columnDelay)));
        const incomingEase = easeOut(local);
        const x = pad + sprite.col * (cellW + gap);
        const fromY = pad + sprite.fromRow * rowStride;
        const toY = pad + sprite.toRow * rowStride;
        const y = fromY + (toY - fromY) * incomingEase;
        this.drawSymbol(sprite.symbolId, x, y, cellW, cellH, {
  ...
  try {
    ...
    if (canvasRenderer) {
      ...
      await pharaohCanvasRenderer.playResult(
        ...
        {
          ...(ultraLiteRenderer
            ? { reel: 820, drop: 220, win: 320, cascade: 360, empty: 70 }
            : liteRenderer ? { reel: 920, drop: 280, win: 480, cascade: 460, empty: 90 } : { reel: 980, cascade: 560 }),
          onStep: (step) => {
            if (slot.spinRunId !== runId) return;
